# Sentropic Transition Plan

This branch records the Sentropic transition at repository level: public README pair, canonical public URL, branch sequencing, and the operational checklist for repo, DNS, redirects, and Scaleway objects.
...
- Public site target: https://sentropic.sent-tech.ca.
...
- Package namespace target: `@sentropic/*`.
...
- `Top AI Ideas` becomes a business case running on Sentropic, not the project name.
- New extracted packages use `@sentropic/*`.
...
2. BR-14c: publish `@sentropic/llm-mesh`.
3. BR-14b: migrate the application LLM runtime onto the mesh.
4. BR-14a: extract `@sentropic/chat`.
...
| --- | --- | --- | --- |
| BR-14c | `feat/llm-mesh-sdk` | Publish the first standalone npm service: `@sentropic/llm-mesh`, an open Vercel AI SDK-like layer for GPT, Claude, Gemini, Mistral, and Cohere access. It covers token-based use, Codex-account use, and prepares later Gemini Code Assist / Claude Code account support. | First |
...
| BR-14a | `feat/chat-ui-sdk` | Extract `@sentropic/chat` from the web, Chrome, and VSCode surfaces. It should depend on the LLM mesh contract rather than application runtime internals. | After BR-14c, can scope in parallel |
| BR-14e | `chore/sentropic-codebase-finalization` | Final non-chat/non-LLM codebase naming sweep: API/UI package names, labels, tests, fixtures, reports, export metadata, and residual-name allowlist. | Before BR-14d |
| BR-14d | `chore/sentropic-transition-ops` | Execute remaining transition operations: repo rename follow-up, DNS/redirect verification, Scaleway containers, registry images, secrets, workflow names, environment variables, dashboards. Preserve redirects and rollback path. | Mandatory after PR-117 ops and BR-14e unless blocked |

## GitHub Repository Rename

Target: `rhanka/top-ai-ideas-fullstack` -> `rhanka/sentropic`.
...
Canonical public site: https://sentropic.sent-tech.ca.

Checklist:

- Create or confirm the `sentropic.sent-tech.ca` DNS record for the UI.
...
- Add new `sentropic` names as aliases or parallel objects where possible.
...
- `sentropic.sent-tech.ca` is documented as the canonical public URL.
