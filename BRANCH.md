...
- [ ] **Lot N-2** UAT
  ...
  - [ ] Owner criterion (2026-09-20): h2a-side `h2a run claude` via gateway with MUSE ACCOUNT ONLY — UPDATE: owner removed the enrolled cloud-code account owner-side 2026-09-20, precondition MET (no verification of the store by conductor — owner statement). Peer ordered to run `h2a run claude --model muse-spark-1.3 --effort low --gw` headless now. Merge blocked until verdict posted.
...
- [x] **Lot N — Final validation** (2026-09-20 conductor re-run post-Lot4: `make typecheck-api lint-api` + `make test-api` ENV=test-feat-muse-enrollment, FINAL_EXIT=0, 0 failures)
  ...
  - [x] Final gate step 2: run/verify branch CI on that PR and resolve remaining blockers. (2026-09-20 CI `35510597132` on PR #593: 3 failures, ALL pre-existing/infra, none attributable to branch — `codex.test.ts` max_output_tokens identical to `origin/main`, zero gateway `src/**` change by branch (BR75-F1, fails on pristine main too); 2× e2e fail at `up-e2e` on `minio/minio` pull denied, same 2 groups failing on main run `35042112802`. Merge needs owner waiver for pre-existing red.)
