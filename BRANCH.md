...
- [ ] **Lot 7 — Slice 4: `JobQueue` extraction (lease, DLQ, heartbeat, idempotency)**
  ...
  - [x] Slice 7.F.2 (real reorg) — move the 3 SQL helpers (`getProcessingCountByClass`, `claimPendingJobsByClass`, `hasAnyPendingJob`) out of queue-manager and behind the `JobQueue` port; `PostgresJobQueue` now owns class-aware count/claim/has-pending SQL and `processJobs()` wires those adapter methods into `runProcessingLoop`. Build-flow green, typecheck-api green, lint-api green (warnings only), job-queue 15/15, replay 19/19, workflow-transition-runtime 6/6, document-summary 4/4. Commit `20496b18`; lint cleanup `XXXX`.
