...
- **BR26-EX14** (finalization, scope exception): `api/tests/api/initiatives-workflow-runtime.test.ts` is added to the touched paths. Reason: the final `make test-api` gate exposed a remaining real OpenAI call in the multi-org workflow-runtime scenario (`matrix_generate`), while the sibling runtime scenarios already mock `generateOrganizationMatrixTemplate`. Impact: test harness determinism only; no production behavior change and no timeout increase. Rollback: revert the added matrix-template mock if this endpoint test is intentionally promoted to provider-live coverage.
...
- **BR26-FB-05** (finalization, severity: `attention`, status: `resolved/deferred`): `rules/testing.md` "AI Flaky Allowlist" enumerates `03-chat.spec.ts`, `03-chat-chrome-extension.spec.ts`, `07_comment_assistant.spec.ts`, `00-ai-generation.spec.ts`, but PR #165 CI run `26265952314` showed the same OpenAI `insufficient_quota` signature propagating through `04-documents-ui-actions.spec.ts:73`, `08-chat-heavy.spec.ts:85`, and `08-documents-summary.spec.ts:17` (downstream of the `document_summary` and `chat_message` executors). Extending the allowlist is cross-cutting and out of BR32 scope; deferred to a dedicated `chore/rules-testing-ai-flaky-allowlist` follow-up.
...
- Final local validation notes (2026-05-21):
  ...
  - PR CI evidence (2026-05-22, run `26265952314` on `6c27bb1b`, https://github.com/rhanka/sentropic/actions/runs/26265952314):
    - API AI suites failing with `insufficient_quota` provider error: `tests/ai/chat-sync.test.ts`, `tests/ai/executive-summary-auto.test.ts`, `tests/ai/chat-tools.test.ts`, `tests/ai/company-enrichment-sync.test.ts`, `tests/ai/initiative-generation-async.test.ts`, `tests/ai/executive-summary-sync.test.ts`.
    - E2E groups failing with downstream provider-quota signatures (`chat_message failed: insufficient_quota`, `document_summary failed: insufficient_quota`): `tests/03-chat.spec.ts` lines 159/203/592/667/684/732/778 (group-c, job `77309733347`); `tests/04-documents-ui-actions.spec.ts:73` (group-b, job `77309733348`); `tests/08-chat-heavy.spec.ts:85` and `tests/08-documents-summary.spec.ts:17` (group-d, job `77309733391`).
    - Cross-check vs `origin/main` recent CI runs `26203690343`, `25974937529`, `25974683403`, `25962961355`, `25961096757`: all green → classification = 0 regression, 0 preexisting, 3 flaky (single root cause = OpenAI billing quota).
    - User sign-off (2026-05-22) is **provisional**: the merge gate still requires one same-commit CI run where the listed AI + E2E signatures pass once the OpenAI quota is restored, per acceptance rule "non-systematic means at least one success on the same commit and same command". Re-run `gh run rerun --failed 26265952314` (or push an empty re-run trigger) is the planned proof step.
