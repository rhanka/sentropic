...
- [ ] **Lot N-2** UAT
  ...
  - [ ] Owner criterion (2026-09-20): h2a-side `h2a run claude` via gateway with MUSE ACCOUNT ONLY — CORRECTION 2026-09-20: conductor's "vacuous" note was WRONG (misread `~/.sentropic/accounts.json`, a stale file whose `claude-code` entries are not enrolled accounts; cloud-code = gemini, and live mesh routing prefers `codex` then `cloud-code`). Cloud-code enrollment state UNDETERMINED — needs owner pointer to the real store. BR75-D6 GO stands, but the precondition check must be redone against the right store. Merge blocked until verdict posted.
