...
## Feedback Loop
- [x] BR600-EX9 — CI 35961364886 05-usecase-detail.spec.ts:854 root cause is test isolation: parallel specs share seeded user A, and `clearLocksForUser` on A's last SSE disconnect in another worker removes the lock held by this scenario. Give the lock/presence scenarios isolated magic-link users (as in the 01-organizations-detail lock-on-leave fixture). Allowed: `e2e/**`. Impact: test-only. Rollback: revert the commit.
- [x] BR600-EX8 — REVERTED (orchestrator design decision after verdict head 3a0b46dfe): conditional lock release and client renewal state machine removed; lock files equal origin/main. Product issue tracked in `.h2a/runs/ci-truthful-build-astra/ISSUE-multitab-lock.md`.
...
- [ ] **Lot 2 — Hidden failure investigation**
  ...
  - [x] Orchestrator decision after verdict head 3a0b46dfe: lock renewal product changes reverted (EX5/EX6/EX8); lock/presence E2E participants isolated per test (EX9). CI retest pending.
