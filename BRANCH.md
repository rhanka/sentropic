...
- [ ] **Lot 7 — Slice 4: `JobQueue` extraction (lease, DLQ, heartbeat, idempotency)**
  ...
  - [x] Slice 7.F.4c (real reorg) — bind `organization_batch_create` executor (no abort branch, no terminal mirror — same pattern as 7.F.4a/b). Added one entry to `executors` in `buildJobRunnerDeps()`; removed `case 'organization_batch_create':` from the legacy in-place switch. JobQueue contract test covers the new binding. Typecheck-api green, replay 19/19, workflow-transition-runtime 6/6, organization-batch-runtime 5/5, job-queue 4/4. Commit XXXX.
  - [ ] Slice 7.F.4d..h — bind remaining 5 simple executors (organization_enrich, matrix_generate, initiative_list, initiative_detail, docx_generate) — same pattern.
