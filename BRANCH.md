...
- **Track B - BR-39b `@sentropic/auth-hono`**
  - Done:
    ...
    - `@sentropic/auth-hono@0.2.0` extends the WebAuthn route-handler contract with an `AuthHonoRouteHandlerError` short-circuit on `prepareRegistrationOptions`/`resolveRegistrationUser`/`resolveAuthenticationOptions` and optional `finalizeRegistration`/`finalizeAuthentication` post-verify hooks — additive, backward-compatible, with 4 new package tests (`BR39b-Q7` package contract resolved).
    - Sentropic `api/src/services/auth/webauthn-adapter.ts` (`BR39b-EX3` extension) exports `authHonoWebAuthnRegistrationService` and `authHonoWebAuthnAuthenticationService`; `generate*Options` delegate to the existing app services, `verify*` throw app-owned for now and will land with the `/verify` rewires.
    - Sentropic `api/src/routes/auth/login.ts` `POST /login/options` now consumes `createAuthWebAuthnAuthenticationRouteHandlers` (inline `resolveAuthenticationOptions` does the email → `userId` lookup); `POST /login/verify` stays app-owned for the next slice.
    - Sentropic `api/src/routes/auth/register.ts` `POST /register/options` now consumes `createAuthWebAuthnRegistrationRouteHandlers` through a `prepareSentropicRegistrationOptions` callback that preserves the email-token validation, legacy display-name fallbacks, existing-user vs new-user branching, first-admin policy, and emits structured `AuthHonoRouteHandlerError` 403s (`invalid_verification_token` / `email_verification_required`) via the new package short-circuit hook. `registration.test.ts` updated to the structured `data.error.code` shape per `BR39b-DEC2`. `POST /register/verify` stays app-owned for the next slice.
