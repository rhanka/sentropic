...
- [ ] **Lot N-2** UAT
  ...
  - [ ] Owner criterion (2026-09-20): h2a-side `h2a run claude` via gateway with MUSE ACCOUNT ONLY — precondition VACUOUS (verified 2026-09-20 both sides: mesh pool holds only 2 `claude-code` accounts, no gemini account to remove, no hand-edit; BR75-D6 GO therefore executes as no-op). Run `h2a run claude --model muse-spark-1.3 --effort low --gw` headless directly. Merge blocked until verdict posted.
