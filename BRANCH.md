...
- `acknowledge` BR-L1c-EX3 — `ui/src/routes/+layout.svelte` and `ui/src/lib/components/Toast.svelte` touched outside the original allowed paths. Reason: the published drawer side must move application content and toast clearance from the matching viewport edge; package-only changes cannot prevent a left drawer from covering the application. Impact: additive layout side contract plus side-aware app padding and toast offset; right-side behavior and no-menu behavior remain unchanged. Rollback: revert the layout-state, app-layout, and toast hunks together.
...
- `attention` — the menu-less path renders identical container classes and publishes an identical layout payload to pre-branch, BUT it is not literally unchanged: it now republishes the layout on a docked/floating toggle where main did not. That is a fix for a latent staleness bug, not a no-op, so "byte-identical" is claimed for the rendered output and the payload shape only. UAT step: open the chat, toggle dock/undock, confirm the page padding and the header burger react immediately.
- `acknowledge` BR-L1c-baseline — owner validated this scope hands-on on 2026-07-28 at commit `405ebdb4902870ff07f8b9153bc1a061fe26f92a`. That commit is the agreed fallback point for the chat placement work: return to it if a later evolution (stacked occupancy in an existing drawer, L5) goes wrong.
- `attention` BR-L1c-drag-1 — KNOWN ROUGH EDGE: starting a header drag IMMEDIATELY after a previous drag committed does not arm. Traced in a real browser: the grip arms on pointerdown, then disarms before the movement threshold is reached, so no drop zones appear; a second attempt works. Each gesture is correct from a settled state (docking, maximising and detaching are all covered by e2e), and docking via the menu then dragging works. Root cause not isolated — an unmatched pointerup/pointercancel reaches the window listener between the press and the threshold. Not fixed rather than half-fixed; the detach e2e therefore docks through the menu, and chaining two drags is explicitly NOT covered.
- `attention` — expected, not defects, during UAT: with a menu-owned LEFT drawer the toast keeps the floating-bubble clearance on the right rather than hugging the panel, and a full-screen placement gets the floating-panel offset. Both keep the toast readable over the chat; neither is a regression introduced by this branch.
