...
- [x] `attention`: Before merge, record Web, Chrome, and VSCode UAT as passed, or record explicit user waiver. Closed on 2026-05-22: root `ENV=dev` UAT was accepted by user after lazy-load and StreamMessage overflow fixes; final automated gates remain in Lot N.
...
- [x] `BR14a-EX4`: Conditional Path touched: `ui/tailwind.config.cjs`. Rationale: Lot 10 moved `StreamMessage.svelte` from `ui/src/lib/components` into `packages/chat-ui/src/components`, but the UI Tailwind content scan still only covered `./src/**/*.{html,js,svelte,ts}`. Impact: package-owned chat UI source is now scanned by the app Tailwind build, restoring generated classes that existed before BR-14a extraction (`max-h-16`, `max-h-24`, and any other package-only utilities). Rollback: remove `'../packages/chat-ui/src/**/*.{html,js,svelte,ts}'` from the content array. Closed on 2026-05-22.
- [x] `BR14a-EX5`: Conditional Paths touched: `README.md`, `TODO.md`. Rationale: Lot N-1 docs consolidation replaces stale public/conductor wording that still named `@sentropic/chat` while BR-14a's accepted package name is `@sentropic/chat-ui`. Impact: docs-only naming correction; no runtime, package metadata, or CI behavior change. Rollback: revert the README/TODO hunks. Closed on 2026-05-22.
- [x] `regression` (Lot N-2 UAT, root commit `70731f09`, fixed in `566b85e3` / root UAT cherry-pick `71746d75`): jobs panel + use case cards — reasoning/tool-call step `auxText` preview overflows (no 64px cap), expanded step bodies overflow (no 96px cap). Repro: trigger any folder generation or background job → expand `QueueMonitor` row, or open a `Brouillon` use case card in `/folders/[id]`. Expected: `auxText` capped at `max-h-16` (64px) with `overflow-y-auto`; expanded step body capped at `max-h-24` (96px). Actual: classes absent from compiled CSS bundle — verified `curl http://localhost:5271/src/app.css | grep -oE '\\.max-h-[0-9]+' | sort -u` returns only `.max-h-28/48/52/56` (used by `ui/src/**`). Root cause: `ui/tailwind.config.cjs:2` content glob is `'./src/**/*.{html,js,svelte,ts}'` — does NOT include `packages/chat-ui/src/**`. Lot 10 (`f0ceea09`) moved `StreamMessage.svelte` into the package without extending the Tailwind scan paths, so `max-h-16` (3 hits: lines 1001/1018/1032) and `max-h-24` (line 968) — present only in `packages/chat-ui/src/components/StreamMessage.svelte` — never get compiled. Fix: add `'../packages/chat-ui/src/**/*.{html,js,svelte,ts}'` to `ui/tailwind.config.cjs` content array. Validation: root `make dev ENV=dev`, `make ps-all ENV=dev`, and `make wait-ready ENV=dev` passed; compiled CSS from `http://ui:5173/src/app.css` contains `.max-h-16` and `.max-h-24`; user accepted root UAT on 2026-05-22.
- [x] `regression` (Lot N-2 UAT, root commit `70731f09`, fixed in `e447c3f9`): session switch lost the progressive lazy-load mechanism. Repro: with several sessions in the list (some with long history), click another session entry. Expected: bottom of the new conversation appears within a few hundred ms while older messages stream in above; a loading indicator covers the gap if any. Actual: previous session stays frozen with zero visual feedback for the full duration of the server-side history fetch (≥2s on long sessions), then the whole new session swaps in one frame. Root cause: commit `4679ca92` ("fix(chat): defer session reveal until staged history is ready", 2026-03-08, predates BR-14a) replaced `selectSession → loadMessages(id, { scrollToBottom: true })` with `selectSession → await fetchSessionHistorySnapshot(id) → atomic apply`. The replacement reads the full NDJSON stream into a snapshot object BEFORE touching any UI state. This bypasses the progressive mechanism that still lives in `loadMessages` (verified present at `AppChatPanel.svelte:3586-3719`): NDJSON consumed line-by-line in `while reader.read()` loop (3663-3673), each item appended to `stagedHistoryTimelineItems` via `stageHistoryTimelineItem` (3415), buffer flushed via `applyHistoryTimelineBlock` when staged height exceeds one viewport (`shouldFlushHistoryStage`, 3419-3426), first batch revealed at bottom (`revealAtBottom: historyTimelineItems.length === 0`, 3657) — so the end of the conversation appears immediately and earlier messages stream in above. `loadMessages` also sets `loadingMessages = true` (3594/3717) which renders the existing spinner during the wait. `selectSession` (3721) bypasses ALL of this: it awaits the FULL snapshot synchronously, never touches `loadingMessages`, and applies everything in one shot. Verified BR-14a refactor did NOT alter `selectSession` semantics — only diffs vs `4679ca92` are `new Set()` → `.clear()` (no functional change) and two added `scan/extractGeneratedFileCards…` loops (sub-ms). The API endpoint `GET /chat/sessions/:id/history?runtimeDetails=summary` (`api/src/routes/api/chat.ts:346`) still streams NDJSON server-side, so the progressive path remains viable. Fix candidate (preferred): rewrite `selectSession` to call `loadMessages(id, { scrollToBottom: true })` directly — that path already implements deferred reveal via `historyHydrationInFlight` + `stagedHistoryTimelineItems` + `applyHistoryTimelineBlock(...,{revealAtBottom})`, plus the loading indicator, plus the progressive batching. The `fetchSessionHistorySnapshot` helper becomes dead code on the switch path (its only remaining caller is `selectSession`). Need to validate the deferred-reveal anti-flash intent from `4679ca92` is preserved by `loadMessages` — examine staged buffer behavior to confirm previous session messages stay visible until first batch lands. Scope: `ui/src/lib/components/chat/AppChatPanel.svelte` (in Allowed Paths). Predates BR-14a but surfaced by Lot N-2 UAT; absorbed into BR-14a final lots per user request. **Fix delivered in `e447c3f9`:** rewrote `selectSession` to consume the NDJSON stream line-by-line, stage items via `stageHistoryTimelineItem`, and on the first viewport-sized batch perform an atomic `performDeferredClear()` (resets `messages`, `historyTimelineItems`, `initialEventsByMessageId`, `runtimeSummaryByMessageId`, `optimisticSteerMessages`, `projectedStreamEventsById`, `projectedAssistantComputationByMessageId`, `loadedRuntimeDetailsMessageIds`, `loadingRuntimeDetailsMessageIds`, `resetLocalToolInterceptionState`, `sessionDocs`, sets `sessionId = id` and `historyHydrationSwapPending = true`) then `applyHistoryTimelineBlock(..., { revealAtBottom: true })` — so the previous session stays visible until the first batch lands, then the new session reveals at the bottom (end-of-conversation first), and older messages stream in above. `loadingMessages = true` set immediately on entry → existing spinner renders during the server wait. Subsequent batches use plain `applyHistoryTimelineBlock` (no `revealAtBottom`) so they prepend above the visible area. Anti-flash from BUG-L6-44 preserved: previous-session messages are NOT cleared until the first batch is staged and `historyHydrationSwapPending` is true (which makes the timeline `class:invisible` per template at line 5032). Dead code removed: `fetchSessionHistorySnapshot` (-128 lines) and `SessionHistorySnapshot` type (-12 lines). Diff: 141 added / 197 removed (net -56 lines). Gates: `make typecheck-ui REGISTRY=local ENV=test-feat-chat-ui-sdk-v2-fixn1` 0 errors / 6 existing warnings; `make test-ui SCOPE=tests/components/chat ENV=test-feat-chat-ui-sdk-v2-fixn1` 26/26 passed across 8 files; `make test-chat-ui ENV=test-feat-chat-ui-sdk-v2-fixn2` 79/79 passed across 19 files. Manual UAT validation remaining on `tmp/feat-chat-ui-sdk-v2` dev (5271) before final UAT sign-off.
...
- [x] **Lot 2 - Package scaffold and public UI contract**
  - [x] Open `BR14a-EX1` for package/workspace metadata if `package-lock.json` or package scripts must change. (Opened — see Feedback Loop line 132; extended to publication wiring as `BR14a-EX1-extended` in Lot 6.)
  ...
  - [x] Lot gate:
    - [x] Run package scoped typecheck target after adding it: `make typecheck-chat-ui ENV=test-feat-chat-ui-sdk-v2` (deferred to Lot 6 under `BR14a-EX1`; closed in Lot 6).
    - [x] Run package tests after adding target: `make test-chat-ui ENV=test-feat-chat-ui-sdk-v2` (deferred to Lot 6 under `BR14a-EX1`; closed in Lot 6).
    - [x] Run `make typecheck-ui API_PORT=9071 UI_PORT=5271 MAILDEV_UI_PORT=1171 ENV=test-feat-chat-ui-sdk-v2` (executed in Lot 3 gate per line 129).
    - [x] Commit package scaffold and tests with selective `git add`, then `make commit MSG="feat: scaffold sentropic chat ui package" ENV=test-feat-chat-ui-sdk-v2` (scaffold landed across `db685645`, `e74031ac`, `78ce6bc4`, `cd8ea4c0` rather than as a single dedicated commit, because UI-generic stores/utils and exports map were rebased on each adoption step).
...
- [x] **Lot 8 - Dependency map and modular contract lock**
  ...
  - [x] Lot gate:
    ...
    - [x] Commit scope/spec plan with selective `git add BRANCH.md spec/SPEC_STUDY_CHAT_UI_SDK_SCOPE.md`, then `make commit MSG="docs: lock chat ui modular refactor contract" ENV=test-feat-chat-ui-sdk-v2` — `2a0f2023`.

- [x] **Lot 9 - Stream hub factory extraction**
  ...
  - [x] Lot gate:
    ...
    - [x] Commit with selective `git add`, then `make commit MSG="refactor: extract stream hub factory to chat ui" ENV=test-feat-chat-ui-sdk-v2-lot9` — `4b66db74`.

- [x] **Lot 10 - StreamMessage package activation**
  ...
  - [x] Lot gate:
    ...
    - [x] Commit with selective `git add`, then `make commit MSG="refactor: activate package stream message" ENV=test-feat-chat-ui-sdk-v2-lot10` — `f0ceea09`.

- [x] **Lot 11a - ChatPanel projection state extraction**
  ...
  - [x] Commit with selective `git add`, then `make commit MSG="refactor: extract chat timeline projection" ENV=test-feat-chat-ui-sdk-v2-lot11a` — `67b0283d`.
  - [x] Run `make down ENV=test-feat-chat-ui-sdk-v2-lot11a`.

- [x] **Lot 11b - ChatPanel draft and composer action state**
  ...
  - [x] Commit with selective `git add`, then `make commit MSG="refactor: extract chat composer draft state" ENV=test-feat-chat-ui-sdk-v2-lot11b` — `244a5004`.
  - [x] Run `make down ENV=test-feat-chat-ui-sdk-v2-lot11b`.

- [x] **Lot 11c - ChatTimeline package component**
  ...
  - [x] Commit with selective `git add`, then `make commit MSG="refactor: extract chat timeline renderer" ENV=test-feat-chat-ui-sdk-v2-lot11c` — `aa81fd51`.
  - [x] Run `make down ENV=test-feat-chat-ui-sdk-v2-lot11c`.

- [x] **Lot 11d - ChatComposer package shell**
  ...
  - [x] Commit with selective `git add`, then `make commit MSG="refactor: extract chat composer shell" ENV=test-feat-chat-ui-sdk-v2-lot11d` — `a0afe924`.
  - [x] Run `make down ENV=test-feat-chat-ui-sdk-v2-lot11d`.

- [x] **Lot 11e - AppChatPanel public wrapper split**
  ...
  - [x] Commit with selective `git add`, then `make commit MSG="refactor: split app chat panel wrapper" ENV=test-feat-chat-ui-sdk-v2-lot11e` — `55dd5ce0`.
  - [x] Run `make down ENV=test-feat-chat-ui-sdk-v2-lot11e`.

- [x] **Lot 11f - AppChatPanel adapter extraction**
...
- [x] **Lot 12a - ChatWidget package layout state**
...
- [x] **Lot 13a - Web host adapter contract**
...
- [x] **Lot 14 - Streaming render resilience and regression guard**
  ...
  - [x] Add UI regression tests in `ui/tests/components/chat/ChatTimeline.test.ts` for stable reasoning/tool/content segment keys across late deltas and steer insertions.
  - [x] Keep UX scope constrained: no new lag UI, no timeout increases, no provider-specific special casing except existing smoothing configuration.
  - [x] Run `make typecheck-chat-ui ENV=test-feat-chat-ui-sdk-v2-lot14` — exited 0.
...
- [x] **Lot N-2 - Full UAT after modular refactor**
  - [x] Confirm `tmp/feat-chat-ui-sdk-v2` worktree is clean and all Lot 11-14 commits are present. Reconfirmed before Lot N-1 docs edits; final docs commit restores clean state.
  - [x] Prepare root UAT branch/worktree so root is commit-identical to `tmp/feat-chat-ui-sdk-v2`; this is Codex-owned, not user-owned. Root UAT was first run through the cherry-picked equivalent `71746d75`; after Lot N-1 docs commit root is realigned to the branch HEAD for commit-identical handoff.
  - [x] Run `make dev ENV=dev` from root only after root points at the UAT commit.
  - [x] Run `make ps ENV=dev`. Equivalent `make ps-all ENV=dev` captured all root services.
  - [x] Run `make wait-ready ENV=dev`.
  - [x] Verify with Playwright that `http://localhost:5173/` renders visible content and has 0 browser console errors. User accepted root UI UAT on 2026-05-22; automated Playwright rerun is deferred to Lot N gates.
  - [x] Verify desktop floating chat widget opens, closes, sends a message, streams reasoning, streams tool calls, streams content, persists final content, and survives refresh. User accepted root UAT on 2026-05-22 after lazy-load and StreamMessage fixes.
  - [x] Verify docked chat panel docks, undocks, preserves session selection, and does not regress layout. User accepted root UAT on 2026-05-22.
  - [x] Verify mobile chat opens as expected full-screen or bottom-sheet surface and composer controls do not overlap. User accepted root UAT on 2026-05-22.
  - [x] Verify new chat session creation from initiative, organization, folder/workspace, and neutral context. User accepted root UAT on 2026-05-22.
  - [x] Verify tool-call rendering, local-tool permission prompts, attachments, context/tool menu, interruption, retry, rollback, copy, feedback, and generated-file download controls. User accepted root UAT on 2026-05-22.
  - [x] Verify SSE resync/history after navigation and page refresh. Lazy-load/session switch regression fixed in `e447c3f9` and accepted by user on root UAT.
  - [x] Verify Jobs tab remains app-owned but integrated: active/failed badges update, `QueueMonitor` opens, expanded job stream history renders through wrapper. StreamMessage cap regression fixed in `566b85e3`; compiled CSS contains `.max-h-16` and `.max-h-24`; accepted by user on root UAT.
  - [x] Verify Chrome extension download endpoint and settings UI still expose a valid package. Covered by user UAT sign-off for BR14a surfaces; automated Chrome E2E remains in Lot N.
  - [x] Verify Chrome side panel/floating chat behavior, runtime permission prompts, `tab_read`, and `tab_action` flows if extension assets are available. Covered by user UAT sign-off for BR14a surfaces; automated Chrome E2E remains in Lot N.
  - [x] Verify VSCode chat webview loads, can send a message, receives streamed output, preserves local-tool permissions, summary/checkpoint flows, host messaging, and auth bridge. Covered by user UAT sign-off for BR14a surfaces; automated VSCode E2E remains in Lot N.
  - [x] Record known reservation only if intermittent provider/network/render slowdowns still eventually deliver ordered content without UI exceptions. No new reservation after the final StreamMessage/Tailwind fix; earlier provider/network slowdowns remain accepted as non-systematic partial-UAT reservation from Lot 7.
  - [x] Record UAT passed for all impacted surfaces, or record explicit user waiver. User message on 2026-05-22: "ok c bon".
  - [x] No merge without UAT passed or waiver.

- [x] **Lot N-1 - Docs consolidation**
  - [x] Update `spec/SPEC_STUDY_CHAT_UI_SDK_SCOPE.md` with actual final package decisions, component boundaries, exported subpaths, and wrapper/adapters.
  - [x] Update `spec/SPEC_STUDY_ARCHITECTURE_BOUNDARIES.md` only if actual package dependencies differ from the study. No edit required: the existing dependency rule (`chat-ui` consumes chat-core wire only, no mesh/server/persistence ownership) still matches the final implementation.
  - [x] Update `spec/SPEC_EVOL_SENTROPIC_BR14_ORCHESTRATION.md` to replace stale `@sentropic/chat` wording with `@sentropic/chat-ui` where BR-14a is described.
  - [x] Update `README.md` and `TODO.md` naming only under an approved `BR14a-EXn` if conductor/public docs must be synchronized in this branch. Done under `BR14a-EX5`.
  - [x] Record BR-07 handoff notes for UI npm/pretest consumption.
  - [x] Record final module inventory for package-owned components/stores, app-owned wrappers/adapters, host-owned Chrome/VSCode code, and app-owned jobs tracking.
  - [x] Run docs-only review using `rg -n "@sentropic/chat|@sentropic/chat-ui|BR-14a|chat-ui" README.md TODO.md PLAN.md spec plan`. Completed before edits; remaining stale `@sentropic/chat` occurrences in `PLAN.md`, `plan/**`, and historical specs are scheduling/historical docs outside BR14a edit scope.
  - [x] Commit docs with selective `git add`, then `make commit MSG="docs: consolidate modular chat ui contract" ENV=test-feat-chat-ui-sdk-v2`.
