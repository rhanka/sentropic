...
- [ ] Lot N: final validation, UAT, PR
  ...
  - [x] `make clean test-e2e API_PORT=8714 UI_PORT=5114 MAILDEV_UI_PORT=1014 ENV=e2e-br14e`
...
- BR14e-F18: Grouped E2E runner status/defaults fixed. Commands: RED then GREEN `make test-ui SCOPE=tests/utils/sentropic-infra-contract.test.ts API_PORT=8714 UI_PORT=5114 MAILDEV_UI_PORT=1014 ENV=test-br14e`; final proof through `make clean test-e2e API_PORT=8714 UI_PORT=5114 MAILDEV_UI_PORT=1014 ENV=e2e-br14e`. Status: acknowledge; grouped Playwright runs now default to `WORKERS=1`, retain `RETRIES=2`, and propagate any group failure through the final make exit code.
...
- BR14e-F21: Final full E2E gate passed. Commands: `make clean test-e2e API_PORT=8714 UI_PORT=5114 MAILDEV_UI_PORT=1014 ENV=e2e-br14e`. Status: acknowledge; command exited 0 across groups 00-07. Observed retries/flaky signatures: group 01 lock/presence released on retry for two tests, group 03 initial chat send visibility released on retry, group 07 matrix lock/presence passed without retry in the final run.
