...
- [ ] **Lot 7 — Slice 4: `JobQueue` extraction (lease, DLQ, heartbeat, idempotency)**
  ...
  - [x] Slice 7.F.3 (real reorg) — settings reload mechanism moved behind `loadQueueSettings` in `packages/flow/src/job-queue-controls.ts`; queue-manager now binds `settingsService.getAISettings()` plus the three runtime setting setters (`maxConcurrentJobs`, `maxPublishingJobs`, `processingInterval`) through hooks. Build-flow green, typecheck-api green, job-queue-settings 2/2, job-queue-controls 3/3, ai-settings 11/11. Commit `XXXX`.
