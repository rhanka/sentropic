...
- [x] **Lot G — Merge-gate fixes (Codex 5.5xhigh BLOCK + Opus 4.8 nits union, PR #371)**
  - [x] G1 (BLOCKER) `src/durable.ts` `#waitCleared('elicitation')`: bind the durable resume like F6 — fetch the elicitation record and require `state==='resumed'` AND `capabilityRef`/`sessionRef`/`actor.sub` match the durable call's capability/session/principal; a foreign-capability/session/principal resumed ref never clears the wait + test (in-memory + FileRecordStore restart).
  - [x] G2 (BLOCKER) `src/durable.ts` idempotency lookup: scope `findByIdempotencyKey(key, scope)` to capability + connectorInstance + tenant + principal + session; a same-key launch from a different context creates a DISTINCT call (collision denied) + test.
  - [x] G3 (HIGH) `src/context.ts` secret transition to non-active: delete the LIVE value from `#values` (resolve fails closed) but DO NOT `redactor.forget` — redaction stays monotonic + test (resolve throws AND old value still redacted post-revoke).
  - [x] G4 (HIGH) `src/digest.ts` + `src/guard.ts` + `src/durable.ts`: audit a stable opaque `idempotencyKeyDigest` (SHA-256 short hash) instead of the raw caller-controlled key + tests (secret-looking key never raw in guard/durable audit); evolved the prior `tool.invoke` raw-key assertion.
  - [x] Opus nit #2 `src/context.ts`: `MockSecretStore.put(key, value, scope?)` threads the real scope (default `connector-instance`) instead of hardcoding it.
  - [x] Lot gate: `tsc --noEmit` PASS + vitest 87/87 across 10 files (prior 80 + 7 new); red-first verified (G1/G2/G3 fail on old logic); host-node ephemeral toolchain (global tsc 5.4.5 / vitest 4.0.18, no repo `node_modules`, root lock/`package.json` untouched).
