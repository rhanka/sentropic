...
- Owner UAT (2026-07-05) — 4 findings, root cause = bypassed app StreamMessage wrapper (labels=$_ + streamClient=streamHub injections lost in extraction): UAT-1/2 raw stream.* label keys → FIXED (ChatPanelShell streamClient prop + labels wired to both StreamMessage mounts); UAT-4 bubble→attachment gap → FIXED (MessageAttachments inline under bubble, before the hover actions row whose reserved height caused the gap); UAT-5 assistant actions always visible → owner-requested hover (module was 'visible always' by design + gold É3 shows visible; changed to group-hover). Assistant full-width design → double consensus (Opus+Codex CONVERGED): assistantLayout 'plain'|'bubble' flag, additive minor, opt-in plain, comments stay bubbles — follow-up lot, not this branch. Spacing polish (assistant→user, user→next) → proposal pending owner: space-y-2→space-y-3 on the list container.
