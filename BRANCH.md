...
- [ ] **Lot 7 — Slice 4: `JobQueue` extraction (lease, DLQ, heartbeat, idempotency)**
  ...
  - [x] Slice 7.F.1 (real reorg) — **outer processing loop**: `processJobs()` outer `while` body moved into new `packages/flow/src/processing-loop.ts` as `runProcessingLoop(deps: ProcessingLoopDeps)` with 10 deps (`getSettings`, `isPaused`, `isCancelAllInProgress`, `setProcessing`, `getProcessingCountByClass`, `claimPendingJobsByClass`, `hasAnyPending`, `notifyJobEvent`, `getJobId`, `processJob`). The 3 inner SQL lambdas (`getProcessingCountByClass`/`hasAnyPending`/`claimPendingJobsByClass`) became `private async` methods on `QueueManager` and are wired into deps. `inFlight: Set<Promise<void>>` and the `sleep(ms)` helper now live inside the package function. `packages/flow/tsconfig.json` lib extended to `["ES2022","DOM"]` (mirrors `packages/llm-mesh`) so the package can use `setTimeout`/`console`. `processJob` (~600 LOC) STAYS in queue-manager and is passed in as `deps.processJob`. Replay 19/19 green, workflow-transition-runtime 6/6, job-queue 3/3.
