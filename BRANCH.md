...
- [ ] `regression` (Lot N-2 UAT, root commit `70731f09`): session switch shows no loading feedback for ≥2s when clicking a different conversation. Repro: with at least two sessions in the list and the current session showing messages, click another session entry. Expected: an immediate loading indicator (spinner) appears while history loads, OR the new session renders progressively. Actual: previous session stays frozen with zero visual feedback for the full duration of the history fetch, then swaps in one frame. Root cause traced to `4679ca92` ("fix(chat): defer session reveal until staged history is ready", 2026-03-08, predates BR-14a). Before that commit, `selectSession` called `loadMessages(id, { scrollToBottom: true })`, which set `loadingMessages = true` (line 3594) and showed the existing spinner. After 4679ca92, `selectSession` calls `await fetchSessionHistorySnapshot(id)` (current `AppChatPanel.svelte:3722`) which blocks on the full NDJSON snapshot before any state mutation — `loadingMessages` is never touched on this path (verified by grep: only `loadMessages` sets it, lines 3594/3717). The API endpoint `GET /chat/sessions/:id/history?runtimeDetails=summary` (`api/src/routes/api/chat.ts:346`) also awaits the full `chatService.getSessionHistory` result server-side before opening the response body — the streaming shape is cosmetic, the wait is server-time. Verified BR-14a refactor did NOT alter `selectSession` semantics — the only behavioral diffs vs `4679ca92` are: `loadedRuntimeDetailsMessageIds = new Set()` → `.clear()` (no functional change), and two added `scan/extractGeneratedFileCards…` loops over `snapshot.initialEvents` / `snapshot.runtimeSummaries` (O(n) per message, sub-millisecond on realistic payloads). Fix candidate: set `loadingMessages = true` at start of `selectSession`, clear in `finally`, so the existing `{#if loadingMessages}` UX renders during the snapshot await. Cleaner option: track a distinct `loadingSessionSwitch` flag so the spinner is contextually anchored to the session list, not the message area. Scope: `ui/src/lib/components/chat/AppChatPanel.svelte` (in Allowed Paths). NOT a BR-14a-introduced regression — predates the branch — but surfaced by Lot N-2 UAT; recommend dedicated `fix/chat-session-switch-loading-indicator` branch unless user opts to absorb into BR-14a Lot N-1 docs lot.
