...
- `BR38b-EX2` CI E2E image exception 2026-05-25: changed `Makefile` and `e2e/Dockerfile` because this branch touched an E2E spec, which changed `E2E_VERSION` even though specs/helpers are runtime-mounted and not baked into the E2E image. The previous image build path also ran `npx playwright install --with-deps chromium` during CI and the PR job remained stuck in `build-e2e`. Impact is CI image-build stability only; runtime and E2E specs are unchanged. Rollback by restoring the old `E2E_VERSION` inputs and Dockerfile install command if the registry requires per-spec E2E image tags again.
