...
- `attention`: 0008 hand edits (drizzle-kit 0.28 cannot emit `NOT VALID`, `VALIDATE CONSTRAINT` or column comments) — after `make db-generate-control` (FK added in `control-schema.ts`, snapshot/journal regenerated, generated `0008_sturdy_wiccan.sql` renamed to `0008_llm_admission.sql`, journal tag updated), the SQL was edited only to (a) append `NOT VALID` to the 3 `cost_ledger` CHECKs, (b) add the 3 `VALIDATE CONSTRAINT` statements, (c) add the 3 `COMMENT ON COLUMN`. The snapshot stays consistent: it records the same 3 CHECKs (validated state is not modelled) and drizzle does not track comments.
- `attention`: NOT VALID + VALIDATE lock effect — drizzle's migrator applies all pending migrations in ONE transaction, and the preceding `ADD COLUMN` / `ADD CONSTRAINT` on `cost_ledger` already hold ACCESS EXCLUSIVE until commit, so inside 0008 the `VALIDATE` scan does not let writes continue; the gain (writes during a SHARE UPDATE EXCLUSIVE validate) requires the validate in a separate transaction/migration, which the single-migration rule excludes. Validation itself is proven (`convalidated = true` on upgraded scratch DB with historical rows); the scan is over all-NULL columns. Replay tested: runner re-run is a journal no-op; a raw re-execution of 0008 fails on the first `ADD COLUMN` (42701, constraints have no `IF NOT EXISTS`) inside a rolled-back transaction and leaves the schema unchanged.
...
- `acknowledge`: superseded by fix round 1 — the 3 `cost_ledger` CHECKs now use `NOT VALID` + `VALIDATE CONSTRAINT` (see the hand-edit and lock items above).
