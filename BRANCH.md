...
- `BR39rL4-Q1` (`resolved`): the host login-page `login_hint`→presetEmail wiring (deferred at first L4 pass for being out of the original Allowed Paths) is now DONE — the remediation launch packet EXPANDED Allowed Paths to include both login pages, and FIX 3 shipped the read in `apps/auth-idp/web/.../auth/login/+page.svelte` + `ui/.../auth/login/+page.svelte` (commit c7b1615f7). §F is fully complete.
- `BR39rL4-REMEDIATION` (`done`): double-consensus review (Codex BLOCK, adjudicated correct) — 5 fixes applied. FIX1 (api/register.ts): C3 uniform invite gate evaluated BEFORE the existing-user short-circuit in prepare AND resolve → invalid `sit_` for existing-verified === unknown === generic `email_verification_required` 403 (no enumeration); a valid invite proceeds for new AND existing-unverified. FIX2 (auth-hono/authorize-handler.ts): normalize both sides of the login_hint guard + force reauth on null/mismatched session email. FIX3 (both host login pages): read login_hint → presetEmail. FIX4 (auth-ui/AuthRegister.svelte): on the generic `email_verification_required` options-rejection, fall back silently to step='email' (only for `sit_`-syntactic invite presets). FIX5 (auth-ui/AuthLogin.svelte.d.ts): add `presetEmail?: string`. Gates: typecheck-auth-hono GREEN; test-auth-hono GREEN (34 files / 149 tests, incl. +2 FIX2 oracle); api gates still BLOCKED by BLOCK1 (root-owned api/node_modules + Make-only npm-ci hook) — C3 oracle + valid-invite-existing-unverified tests WRITTEN in api/tests/api/auth/registration.test.ts, run pending BLOCK1 recovery.
- `BR39rL4-Q2` (`attention`): FIX4's parenthetical "verify both REGISTER host pages only set preset/skip on a SYNTACTIC `sit_` token" would touch `apps/auth-idp/web/.../auth/register/+page.svelte` + `ui/.../auth/register/+page.svelte`, which are NOT in the remediation Allowed Paths (only the LOGIN pages were added). Left untouched (strict scope). Residual: a non-`sit_` `sentropic_invite_token` URL param would still set `skipEmailVerification=true` and open AuthRegister on step='webauthn'; the server then rejects (generic 403) and the component now falls back to step='email' (FIX4 component-side covers the user-recovery path), so the user is NOT stuck. Tightening the host register pages to ignore non-`sit_` tokens is defense-in-depth. Conductor: approve a 1-line Allowed-Paths extension (2 files, mirror of the login-page change) or land in a follow-up.
...
- **Host REGISTER-page non-`sit_` token tightening** (`apps/auth-idp/web/.../auth/register/+page.svelte` + `ui/.../auth/register/+page.svelte`): defense-in-depth from FIX4 (ignore a `sentropic_invite_token` URL param unless it is `sit_`-prefixed). Out of the remediation Allowed Paths (only LOGIN pages were added). Component-side fallback (FIX4) already prevents the user from getting stuck; this is hardening only. See Feedback `BR39rL4-Q2`.
...
- [x] **Lot 5 — AuthLogin presetEmail for login_hint**
  ...
  - [x] Host login-page wiring (`apps/auth-idp/web/.../auth/login/+page.svelte`, `ui/.../auth/login/+page.svelte`) DONE in remediation FIX3 (Allowed Paths expanded by the remediation packet). Resolves `BR39rL4-Q1`.

- [x] **Lot 6 — Tests**
  - [x] `packages/auth-hono/tests/oauth-authorize-login-hint-invite.test.ts` (8 tests, GREEN): invite present → registerUrl; login_hint alone → loginUrl w/ hint; presence-only (no validity leak); same-user session → continue; session email≠login_hint → force switch; no-registerUrl fallback; +FIX2 oracle: null session email → force reauth; mixed-case session email == hint → no reauth.
  ...
  - [x] `api/tests/api/auth/registration.test.ts` (10 invite tests, WRITTEN — run blocked by BLOCK1): all invalid invite states → identical generic `email_verification_required` (no `invalid_invite`); valid invite → options (email step skipped); +remediation C3 ORACLE: invalid `sit_` for EXISTING-VERIFIED === UNKNOWN (identical status+body); different-email-bound invite indistinguishable existing-verified vs unknown; valid invite → EXISTING-UNVERIFIED proceeds.

- [ ] **Lot N — Final validation**
  - [x] `make typecheck-auth-hono ENV=test-idprpl4` GREEN; `make test-auth-hono ENV=test-idprpl4` GREEN (149 tests, 34 files; remediation re-run).
