...
- [ ] **Lot 2 — `client_credentials` grant on token endpoint (stateless)**
  ...
  - [ ] `packages/auth-hono/src/oauth/wellknown-handler.ts`: `grant_types_supported` + `token_endpoint_auth_methods_supported` per `BR39d-D9`.
  - [ ] If `findServiceClient` is undefined on the port, `client_credentials` ⇒ `unsupported_grant_type` (`BR39d-D8`).
  - [ ] Lot gate:
    - [ ] `make typecheck-auth-hono` + `make lint-auth-hono`
    - [ ] **auth-hono tests**: `packages/auth-hono/tests/oauth-client-credentials.test.ts` — happy (Basic + POST), DPoP-bound happy (`cnf.jkt`, `token_type=DPoP`), no-`scope` ⇒ all allowed, errors: wrong secret (`invalid_client`), scope superset (`invalid_scope`), revoked client, unknown `resource` (`invalid_target`), missing `resource` with >1 indicator (`invalid_target`), 0 indicators + no resource (`invalid_target`). Update `oauth-wellknown` test for new metadata. Assert NO `oauth_tokens` row written.
