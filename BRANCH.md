...
## Feedback Loop
- [x] BR600-EX8 — re-validation verdict (head 9186a871c): late renewal cleanup could delete a lock handed over by an admin, and a pending renewal notification could re-arm renewal after release. Allow an optional `lockId` on `DELETE /locks` (`api/src/routes/namespaces/locks-{contracts,ports,mutation-routes}.ts`, `api/src/services/lock-service.ts`) that deletes only that lock while held by the caller, no admin override; `releaseLock` lockId parameter in `ui/src/lib/utils/object-lock.ts`; helper and five pages move release suppression into `lock-renewal.ts`; tests in `api/tests/{unit/lock-service,api/locks}.test.ts` and `ui/tests/utils/lock-renewal.test.ts`. Impact: compensation can never remove another holder; release sticks until another holder or explicit acquisition. Rollback: revert the commits.
- [x] BR600-EX7 — verdict finding 2: `e2e/tests/10-cluster-mesh-control-plane.spec.ts` had no lane. Under BR600-EX1, add group `10` to the default `E2E_GROUPS` and CI lane group-a, plus `make check-e2e-inventory` (shell script `e2e/scripts/check-spec-lanes.sh`) run in the always-executed `changes` job. Impact: spec 10 blocks CI; an unassigned numbered spec fails CI. Rollback: revert the commit.
- [x] BR600-EX6 — independent test verdict (head 5cba6a0e1) finding 1: the ping and periodic renewal re-acquire a lock after an explicit release whose notification was missed. Allow a lock-renewal intent helper `ui/src/lib/utils/lock-renewal.ts`, its unit test `ui/tests/utils/lock-renewal.test.ts`, and its use in the five lock pages. Impact: explicit release invalidates renewal and pending refresh responses; recovery of a held lock is unchanged. Rollback: revert the commit.
- [x] BR600-EX5 — CI 35961364886 05-usecase-detail.spec.ts:854: B acquires the initiative lock (POST /locks 201) 430 ms after A acquired it; no DELETE, accept or force-unlock in between, so the only remover is `clearLocksForUser` on user A last-SSE disconnect from a parallel context, while A page had not yet opened its SSE and missed the lock_update. Allow the ping handler of the five lock pages in `ui/src/routes/{initiative,folders,organizations}/[id]/+page.svelte`, `ui/src/routes/{matrix,dashboard}/+page.svelte` to re-assert a held lock via the existing `refreshLock`. Impact: one idempotent lock POST per ping for the holder only. Rollback: revert the commit.
...
- [ ] **Lot 2 — Hidden failure investigation**
  ...
  - [x] CI 35961364886 exposed lock loss when a parallel session of the same user closes before the holder page SSE is live; holder pages now re-assert their lock on the post-subscription ping. CI retest pending.
  ...
  - [x] CI 35999807751 first run of lane 10: session-control bodies sent obsolete `commandId`, rejected 400 before the 401 fence; renamed to `commandRef` (request input only, registered). CI retest pending.
