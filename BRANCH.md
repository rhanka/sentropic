...
- [x] `BR75-RV70` — `acknowledge` — Fable Lot 26 C2 is closed by a DB-backed regression that authenticates owner/actor A against distinct workspace B with no A membership, drives valid workspace/admin and folder/editor import manifests to exact 403 `{message:"Insufficient permissions"}`, and proves after each denial that the workspace set, B membership/domain rows and local artifact root are unchanged; the scoped API Compose gate passes 1 file/8 tests. Fresh C1 execution remains pending before final condition closure.
