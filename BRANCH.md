...
- [ ] `BR75-RV22` — `attention` — Fable Lot 8 G1: the process-lifetime gateway route plane retains every prepared plan because settlement hooks do not evict `plans`; close with lifecycle-accurate eviction plus a unit regression proving settled plans are unavailable.
- [ ] `BR75-RV23` — `attention` — Fable Lot 8 G5: add changed `packages/llm-gateway/tests/route-flow-core.test.ts` to the Lot 8 test inventory.
...
- [ ] `BR75-RV25` — `attention` — Fable Lot 8 G6: bump `@sentropic/llm-gateway` from patch line 0.13.x to minor `0.14.0` after verifying the published version remains lower.
