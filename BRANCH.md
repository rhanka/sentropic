...
- [ ] **Lot 20 — `/locks` TARGET collaboration router extraction**
  ...
  - [ ] Add `api/src/routes/namespaces/locks.ts` with independently injected neutral lock/presence/authz ports and a domain stream port; product adapters retain the canonical DB/service authority.
  - [ ] Register the exact enumerated lock route fence in the composition registry, with no `use('*')` or `authPaths: null`; add live-router fence/mount and adversarial route-mutation proofs.
  - [ ] Before deletion, shadow deterministic reads and validate mutation intent without duplicate effects; select one author, verify the exact rollback checkpoint, then delete `api/src/routes/api/locks.ts` and prove the survivor/dual route sweep is empty.
  - [ ] Prove authorization and workspace isolation for every lock/presence object scope, expiry, unlock request, cross-workspace visibility refusal and disconnect/presence cleanup without creating a second lock authority.
  ...
  - [ ] E2E updated: `e2e/tests/05-usecase-detail.spec.ts` lock scenario; execution remains deferred to Lot 33 and is not claimed in Lot 20.
  ...
  - [ ] Internal gates: C1 one lock/presence author; C3/A5 injectable module; cold boot/smoke and the `BR75-RV53` external probe matrix pass or record explicit operational unavailability.
