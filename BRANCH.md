...
- [x] `BR75-RV43` — `acknowledge` — Fable Lot 14 N6 is folded into the `/track` shadow discipline: with a synthetically pinned compatible provider, deterministic evidence/cursor reads each call one read author, while a fully typed valid owner-signature intent sent during the read-only shadow receives 404 and an effect-boundary spy remains at zero calls.
