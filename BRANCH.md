...
- **BR37-EX5** (status: `accepted`): Conditional `Makefile` change for `gh-k8s-secret`, `gh-k8s-secret-check`, `gh-k8s-rerun-deploy`, and neutral `Scaleway Kubernetes` block naming. Reason: encode the operator path for creating `KUBECONFIG_B64` without relying on chat history or ad hoc shell commands. Impact: append-only GitHub CLI helper targets; no secret value is printed or written to disk. Rollback: remove the `gh-k8s-*` targets and restore the previous block heading.
...
- **BR37-FL12** (severity: `blocked`, status: `open`): Branch push CI run 26159456218 published the api/ui images successfully, then `deploy-k8s` failed because GitHub Actions secret `KUBECONFIG_B64` was missing or empty. Direct assistant upload is policy-blocked because it transmits kubeconfig credentials to GitHub. Operator path is now encoded as Make-only targets: `make gh-k8s-secret KUBECONFIG=$HOME/.kube/poc.yaml ENV=test-feat-deploy-poc-k8s`, `make gh-k8s-secret-check ENV=test-feat-deploy-poc-k8s`, then `make gh-k8s-rerun-deploy GH_DEPLOY_RUN_ID=26159456218 ENV=test-feat-deploy-poc-k8s`.
...
- Merge gate:
  ...
  - [ ] Live Kubernetes UAT passed, or explicit user waiver recorded.
...
- [ ] **Lot 2.5 — CI/CD redressement (SCW Registry + Kubernetes rollout)**
  ...
  - [x] Add Make-only GitHub Actions kubeconfig helper targets: `gh-k8s-secret`, `gh-k8s-secret-check`, and `gh-k8s-rerun-deploy`. (BR37-EX5)
  ...
  - [ ] User action: run `make gh-k8s-secret KUBECONFIG=$HOME/.kube/poc.yaml ENV=test-feat-deploy-poc-k8s`, then `make gh-k8s-secret-check ENV=test-feat-deploy-poc-k8s`, to create GitHub Actions secret `KUBECONFIG_B64`.
  - [ ] User action after secret creation: run `make gh-k8s-rerun-deploy GH_DEPLOY_RUN_ID=26159456218 ENV=test-feat-deploy-poc-k8s` or rerun failed CI jobs from the GitHub UI.
...
- [ ] **Lot 3 — Live poc-k8s UAT**
  ...
  - [ ] Verify Kubernetes pull-secret `sentropic-registry` exists in namespace via `kubectl -n sentropic get secret sentropic-registry`.
