...
## Feedback Loop
- BR-EX2: touch api source + tests outside `packages/llm-mesh/**` for the owner-approved OpenAI default cutover to GPT-5.6 Luna: `api/src/services/model-selection-legacy.ts` (repoint `gpt-5.2 → gpt-5.6-luna`, add `gpt-5.5 → gpt-5.6-luna`; `gpt-4.1-nano` stays the base default; `gpt-5.5` stays explicitly selectable — only default resolution migrates) + tests `model-selection-legacy.test.ts` / `models.test.ts` / `ai-settings.test.ts` / `me.test.ts`. Owner chose FULL migration (`gpt-5.5` becomes Luna everywhere the runtime resolves it): `resolveDefaultSelection` (llm-runtime) migrates `gpt-5.5` → `gpt-5.6-luna`, so the Codex-transport trigger moves too — `api/src/services/llm-runtime/index.ts` (`selectedModel === 'gpt-5.5'` → `'gpt-5.6-luna'`) + `api/src/services/chat-service.ts` (context budgets for the 5.6 models) + `chat-service-tools.test.ts` (gpt-5.2 cutover result → luna). Impact: `gpt-5.5` default/runtime selection now resolves to Luna; Codex routing follows. Rollback: revert the cutover rules + the runtime/budget/assertion edits.
