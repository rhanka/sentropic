...
- [x] **Lot 2a — Makefile foundation (install/build/typecheck/pack)**
  ...
  - [x] Lot gate:
    - [x] `git diff Makefile` shows only additive `.PHONY` blocks for new foundation targets.
    - [x] `make commit MSG="feat(make): add internal-packages install/build/typecheck/pack targets"`.

- [x] **Lot 2b — Makefile publish lanes (OIDC + token)**
  - [x] Add `publish-contracts` (OIDC) + `publish-contracts-token` (bootstrap NPM_TOKEN).
  - [x] Add `publish-events` (OIDC) + `publish-events-token`.
  - [x] Add `publish-chat-core` (OIDC) + `publish-chat-core-token`.
  - [x] Add `publish-flow` (OIDC) + `publish-flow-token`.
  - [x] Lot gate:
