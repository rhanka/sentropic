...
- [ ] **Lot 7 — Slice 4: `JobQueue` extraction (lease, DLQ, heartbeat, idempotency)**
  ...
  - [ ] Slice 7.E.2 — **Dispatch graph body** (`dispatchWorkflowTransitions` ~100 lines + `dispatchReadyWorkflowJoins` ~45 lines + `dispatchWorkflowTask` ~200 lines + `dispatchTaskCompletion`). Blocked by `BR26-FB-03` — the hook-based move scales poorly past 4 hooks. Needs functional-decomposition or run-store-aware adapter design.
  - [ ] Slice 7.F — **Lease / processing loop** (`processJobs()` ~125 lines + `processJob` ~600 lines + heartbeat + DLQ retry + `notifyJobEvent` + settings reload + class-based concurrency). The bulk of `queue-manager.ts` (~3500 lines of the file). Pulls in every executor binding (`organization_enrich`, `initiative_*`, `executive_summary`, `chat_message`, `document_summary`, `docx_generate`, …). Same design issue as `BR26-FB-03`.
