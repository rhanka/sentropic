...
- [ ] **Conditional Paths (allowed only with explicit exception when not already listed in Allowed Paths)**
  ...
  - [x] `api/vitest.config.ts`, `ui/vitest.config.ts`, `packages/chat-core/vitest.config.ts` and `packages/chat-server/vitest.config.ts` through `BR75-EX6` only to relocate Vitest caches outside mounted `node_modules`.
- [ ] **Exception process**
...
- [x] `BR75-EX6` — `acknowledge` — reason: recurring containerized Vitest runs create `api/node_modules/.vite`, `ui/node_modules/.vite` and package-level caches such as `packages/chat-core/node_modules/.vite` as uid/gid `65534:65534`, then the next install-backed gate fails with `EACCES rmdir`; paths: `api/vitest.config.ts`, `ui/vitest.config.ts`, `packages/chat-core/vitest.config.ts` and `packages/chat-server/vitest.config.ts`; impact: test-cache location moves to package-specific `/tmp` directories only, with zero runtime or product behavior change; rollback: revert the four `cacheDir` settings; disposition: owner-authorized for this BUILD LEG, extended to the chat packages after recurrent package-level F3 evidence, with no further `make clean-node-modules` use.
