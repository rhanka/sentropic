...
- [x] B0-A4 | Branch: Lot D | Owner: control-schema owner + conductor | Severity: design | Status: decided 2026-09-25 by conductor (reviewer recommendation accepted; B2 must add a concurrency test: two overlapping parallel writes, exactly one succeeds) | G1a is `0008_llm_admission.sql`; pricing non-overlap choice blocks B2 start and must be settled before `make db-generate-control`. PROPOSED (reviewer, not decided): unique `(provider_id, model_id, effective_from)` plus single-writer checked insert (predecessor `FOR UPDATE` and overlap check in the same transaction, owned by B2); no `btree_gist`. Rationale: no control migration creates an extension today; per-tier extension provisioning would burden irreversible G1a.
...
- [x] B0-D1 | Branch: Lot D | Owner: conductor | Severity: release sequencing | Status: decided 2026-09-25 by conductor for design: option A two-phase train; publishing mesh 0.22.0 and gateway 0.19.0 awaits an explicit owner GO (outside the conductor mandate), so phase 1 stops at verified candidates until then | Workspace coupling: mesh 0.22.0 or gateway 0.19.0 on main before cluster 0.13.0 ranges is expected to fail validate-cluster-mesh, and cluster 0.13.0 cannot pass its registry-pinned packed test before publication. Option A: one conductor train branch with same-PR sibling tarballs and EX10; option B reopens D-N6. PROPOSED (reviewer, not decided): option A as a two-phase train (phase 1 mesh 0.22.0 → gateway 0.19.0 with EX10; phase 2 refresh cluster `selected`/`selected-session` qualification lockfiles, qualify, publish cluster 0.13.0), because `test-lazy-package` installs the committed lockfile (`ci.yml:548`) before `pack-candidate-siblings`; option B rejected. Rationale: keeps main green; blocks merge sequencing of Waves 1-3, not development; awaits conductor decision.
...
## Plan / Todo (lot-based)
- [x] **Lot D — B0 fix round 1 (reviewer verdict: approve with listed fixes)**
  ...
  - [x] Findings 11-12 in 12.6-12.7: per-row test obligations, narrowed B3c grant, B2-after-B1 hard gate, B1 extraction/target definition, coherent release proof artifact; B0-D1/B0-A4 reviewer recommendations recorded as proposed.
