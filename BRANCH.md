...
- [ ] **Lot 7 — Slice 4: `JobQueue` extraction (lease, DLQ, heartbeat, idempotency)**
  ...
  - [x] Slice 7.F.4d-f (real reorg) — bind no-jobId simple executors (`matrix_generate`, `initiative_list`, `initiative_detail`). Added entries to `executors` in `buildJobRunnerDeps()`; removed the three matching cases from the legacy in-place switch. JobQueue contract test covers the new bindings. Typecheck-api green, replay 19/19, workflow-transition-runtime 6/6, initiatives-workflow-runtime 4/4, job-queue 7/7. Commit XXXX.
  - [ ] Slice 7.F.4g-h — bind remaining 2 jobId-dependent simple executors (`organization_enrich`, `docx_generate`) after extending the runner executor context explicitly.
