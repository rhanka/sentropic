...
- [x] `BR75-SG12` — `acknowledge` — Fable Lot 7 F3 is reproduced: a containerized API Vitest run creates `api/node_modules/.vite` as uid/gid `65534:65534`, and the next install-backed gate fails with `EACCES rmdir api/node_modules/.vite/vitest`. The durable `api/vitest.config.ts` cache relocation is outside Allowed Paths, so this build leg runs the sanctioned `make clean-node-modules` guard before every install-backed gate and re-bootstraps the API stack before scoped API tests; the guarded Phase A typecheck/lint gate is green and no configuration exception is authorized.
