...
- `BR39c-D20` `decided`: KEK + first signing key bootstrap automatisé. Lot 0 step: générer `OAUTH_SIGNING_KEK=$(openssl rand -base64 32)` dans `.env` worktree + `.env` pour `test-feat-auth-oidc` et `e2e-feat-auth-oidc`. Lot 4 ajoute un make target `make oauth-init-keys ENV=...` qui invoke `jwksAdapter.generateAndStoreNewKey()` sur fresh DB. Le même target tourne en UAT seed (Lot N-2).
- `BR39c-D21` `decided`: `client_secret_hash` algorithm = Argon2id via existing `ports.tokens.hashSecret` (convention BR-39b). Test introspect avec wrong secret → 401.
...
- `BR39c-D29` `decided`: `.env.example` promoted to Allowed Paths (drop `BR39c-EX3`). Update inline dans Lot 4 quand on touche les env-adjacent files. OAuth env vars documentées : `OAUTH_SIGNING_KEK`, `PUBLIC_BASE_URL` (issuer), `OAUTH_ACCESS_TOKEN_TTL_SEC` (default 3600), `OAUTH_ID_TOKEN_TTL_SEC` (default 3600), `OAUTH_AUTHORIZATION_CODE_TTL_SEC` (default 60), `OAUTH_DPOP_IAT_SKEW_SEC` (default 60).
- `BR39c-D30` `decided`: CI lane vérification AVANT toute implémentation. Lot 0 add explicit step: `grep -nE 'packages/auth-hono/tests' .github/workflows/ci.yml` + verify glob picks up `oauth-*.test.ts`. Si non, raise `BR39c-EX1` au Lot 0 (pas mid-flight) avec edit ci.yml exact.
