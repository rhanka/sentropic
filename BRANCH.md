...
- [ ] **Lot 3 — UAT-proof S3: 3 missing e2e specs + org-update + loop-guard salvage proofs**
  ...
  - [x] Part C.1 loop-guard breaker unit: `make test-api-unit SCOPE=tests/unit/chat-service-tools.test.ts API_TEST_WORKERS=1 ...` -> 1 file / 17 tests, 17/17 PASS (9.17s, workers=1).
  - [x] Part C.2 multi-step tool workflow: `00-ai-generation.spec.ts` test "devrait générer des cas d'usage depuis l'accueil et vérifier les références" drives 3 sequential chat tool exchanges (read_usecase -> update_usecase_field with persisted 'Test E2E' field assertion -> web_extract), green post-fix per Lot 2 (2/2 PASS RETRIES=0); `08-chat-heavy.spec.ts` "uploads a composer file attachment and lets chat tools cite it" green in same battery (rc=0).
  - [x] Part C.3 DOCX proven (03-dashboard green); PPTX = Part A.3.
  - [x] Lot gate: Lot 3 diff is e2e specs + BRANCH.md only (zero ui/src change; typecheck-ui unchanged from Lot 2 green) + `make down ... ENV=test-uat`.
