...
- [x] `BR75-EX6` — `acknowledge` — reason: recurring containerized Vitest runs and the UI dev server create `api/node_modules/.vite`, `ui/node_modules/.vite` and package-level caches such as `packages/chat-core/node_modules/.vite` as uid/gid `65534:65534`, then the next install-backed gate fails with `EACCES rmdir`; paths: `api/vitest.config.ts`, `ui/vitest.config.ts`, `packages/chat-core/vitest.config.ts`, `packages/chat-server/vitest.config.ts`, `packages/llm-mesh/vitest.config.cjs` and `ui/vite.config.ts`; impact: test-cache and UI dev-server cache locations move to package-specific `/tmp` directories only, with zero product behavior change; rollback: revert the six `cacheDir` settings; disposition: owner-authorized for this BUILD LEG, extended to llm-mesh after its first scoped Hono test reproduced the same `nobody:nogroup` cache failure; the CommonJS config avoids Vite's ESM `.vite-temp` bootstrap under mounted `node_modules`, with no `make clean-node-modules` use.
