...
- [ ] `BR75-RV53` — `attention` — Fable Lot 19 condition 1 requires a fresh external cold-stack probe on the reserved Lot environment when operational: health 200, anonymous `/streams/active` 401, disabled `/cli` 404 and legacy `/command`, `/commands`, `/terminal`, `/shell` all 404. Record the actual result or explicit runtime unavailability; never substitute in-process evidence.
