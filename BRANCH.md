...
- [ ] **Lot 8 - Dependency map and modular contract lock**
  - [ ] Produce a concrete import/dependency map for:
    - `ui/src/lib/components/ChatPanel.svelte`
    - `ui/src/lib/components/ChatWidget.svelte`
    - `ui/src/lib/components/StreamMessage.svelte`
    - `ui/src/lib/stores/streamHub.ts`
  ...
  - [ ] Use graphify only if the manual import map remains ambiguous; any graphify output must be treated as analysis evidence and must not replace the file-level refactor plan.
  - [ ] Update `spec/SPEC_STUDY_CHAT_UI_SDK_SCOPE.md` with the final module boundary:
  ...
  - [ ] Decide package component API and write it before code movement:
