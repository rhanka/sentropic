...
- [ ] `regression` (Lot N-2 UAT, root commit `70731f09`): jobs panel + use case cards — reasoning/tool-call step `auxText` preview overflows (no 64px cap), expanded step bodies overflow (no 96px cap). Repro: trigger any folder generation or background job → expand `QueueMonitor` row, or open a `Brouillon` use case card in `/folders/[id]`. Expected: `auxText` capped at `max-h-16` (64px) with `overflow-y-auto`; expanded step body capped at `max-h-24` (96px). Actual: classes absent from compiled CSS bundle — verified `curl http://localhost:5271/src/app.css | grep -oE '\\.max-h-[0-9]+' | sort -u` returns only `.max-h-28/48/52/56` (used by `ui/src/**`). Root cause: `ui/tailwind.config.cjs:2` content glob is `'./src/**/*.{html,js,svelte,ts}'` — does NOT include `packages/chat-ui/src/**`. Lot 10 (`f0ceea09`) moved `StreamMessage.svelte` into the package without extending the Tailwind scan paths, so `max-h-16` (3 hits: lines 1001/1018/1032) and `max-h-24` (line 968) — present only in `packages/chat-ui/src/components/StreamMessage.svelte` — never get compiled. Chat surface looks unaffected because `variant='chat'` final content uses `<Streamdown content={displayContent}>` without max-h, and reasoning panel stays collapsed by default (`initiallyExpanded={false}`), masking the missing cap from view. Fix applied in working tree under `BR14a-EX4`: add `'../packages/chat-ui/src/**/*.{html,js,svelte,ts}'` to `ui/tailwind.config.cjs` content array. Validation pending: rerun branch dev and verify `max-h-16` / `max-h-24` are present in compiled CSS and that folder/use-case/job StreamMessage cards cap vertically again.
