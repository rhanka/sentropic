...
- [x] BRLG2-CONSUMER | acknowledge | Owner: consumer conductor (h-cond) | CLOSED 2026-09-25. Evidence (h-cond, h2a consumer conductor): candidate tarball d4e28ff4c444a3bf2e8c3899e725412006150402df02b3a5873268bc9017da30, 2026-09-25T01:26:53Z, isolated h2a worktree @4ba484ef, ranges unchanged, nested 0.17.1 copies removed so 0.18.0 resolved everywhere. Real code WITHOUT annotation: packages/h2a-runtime `tsc -b --force` exit 0; apps/llm-gateway `tsc --noEmit` exit 0. Control of the control: a deliberate probe (`reason` on the ok:true branch) yields `packages/h2a-runtime/src/llm-gateway-runtime/index.ts(107,11): error TS2322` (the typer guards the port); an earlier green was invalid (nested 0.17.1) and was detected by that probe. h2a compiles without annotation because its sites are `const callerAuth: CallerAuthPort = {…}` (contextual typing). Code equivalence d4e28ff4 → 5cb35819eefad7cd01c947f1bc0d9be0fb4fa5568ca08b52be1829fc8e03fbc7 verified independently by the conductor and by h-cond (139 files, 69 .js/.d.ts/package.json identical, only package/README.md differs); the post-merge candidate (HEAD 17a927eb7) has the identical SHA 5cb35819…, so no re-run. Duplicate-mesh E8 qualification with cluster-mesh is carried into the cluster-mesh 0.12.0 lazy-surface lot (Lot E), not a gateway 0.18.0 release gate.
