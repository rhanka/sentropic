...
- [ ] **Conditional Paths (allowed only with explicit exception when not already listed in Allowed Paths)**
  ...
  - [x] `api/vitest.config.ts`, `ui/vitest.config.ts`, `packages/chat-core/vitest.config.ts`, `packages/chat-server/vitest.config.ts` and `ui/vite.config.ts` through `BR75-EX6` only to relocate Vitest and UI dev-server caches outside mounted `node_modules`.
- [ ] **Exception process**
...
- [x] `BR75-EX6` — `acknowledge` — reason: recurring containerized Vitest runs and the UI dev server create `api/node_modules/.vite`, `ui/node_modules/.vite` and package-level caches such as `packages/chat-core/node_modules/.vite` as uid/gid `65534:65534`, then the next install-backed gate fails with `EACCES rmdir`; paths: `api/vitest.config.ts`, `ui/vitest.config.ts`, `packages/chat-core/vitest.config.ts`, `packages/chat-server/vitest.config.ts` and `ui/vite.config.ts`; impact: test-cache and UI dev-server cache locations move to package-specific `/tmp` directories only, with zero product behavior change; rollback: revert the five `cacheDir` settings; disposition: owner-authorized for this BUILD LEG, extended to the chat packages after recurrent package-level F3 evidence and to the UI dev server after Fable Lot 9 reproduced the residual cache, with no further `make clean-node-modules` use.
