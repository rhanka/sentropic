...
- `acknowledge` BR75-D9 (2026-09-22, owner GO): T7 + T15 GREEN live, scratch `/tmp` only, nothing committed except this note. T7 refresh soak 12/12 stable over ~3min (`MuseEnrollmentProvider.refresh()` on the real `auth.json`, same `acct_muse_1ce759f4422d`, token present, zero Meta calls). T15 quota soak 3/3 `muse-spark-1.3` minimal served `ok` spaced 60s with usage (in 12, out 146-197) on a seat-minted key — muse quota, not claude. Lesson: raw CLI token 401s on serve, minted key required (matches device-flow design). Seat still `active` post-soak. ALL TESTS GREEN (T1-T19 + MUSE_API_KEY secret).
