...
- [x] `BR75-RV57` — `acknowledge` — Lot 21 root-mounts the transport-only `/business` module as the sole selected author `business-domain-hono-module` behind 38 exact `BUSINESS_PATHS`. Eight neutral router ports isolate transport from product database/schema/services; product adapters retain the canonical organization, folder, initiative, solution, product, proposal, bid and view-template authorities. Pre-deletion safe-read parity and one candidate-only organization effect are pinned to `2d29d125e`; the exact `legacy-api-business-v1`/`legacy-api-business-routers` rollback fails closed, all eight legacy route files/imports/mounts are absent, and `/initiatives`, compatible `/use-cases`, distinct `/bids` and `/proposals`, plus document-owned DOCX behavior remain explicit. API typecheck and lint pass with 0 errors/208 baseline warnings; the cutover gate passes 8/8, relevant API/security/fence coverage passes 195/195, and initiative unit coverage passes 12/12. UI typecheck and lint pass with 0 errors/6 baseline warnings; the three business stores pass 48/48 and template coverage passes 32/32. A fully stopped final candidate cold-builds to healthy, smoke passes 4/4, seven external business/health/DOCX probes return their exact 200/401/404 statuses, and the project is stopped afterward. The schema and control-migration diff from `36a93f2b0` is empty, leaving `0007_cluster_mesh_r13.sql` as the only Cluster Mesh control migration. Browser E2E remains deferred to Lot 33 and is not claimed executed.
...
- [x] **Lot 21 — `/business` TARGET business router extraction**
  ...
  - [x] Shadow reads/validated mutation intent, select one business author, prove rollback and delete replaced route files `organizations.ts`, `folders.ts`, `initiatives.ts`, `solutions.ts`, `products.ts`, `proposals.ts`, `bids.ts`, `view-templates.ts`.
  ...
  - [x] Tests new: `api/tests/api/cluster-mesh-business-cutover.test.ts` passes 1 file/8 tests, including exact fence enumeration, live negative route mutation, author/rollback fail-close, injected exactly-one effect and transport isolation.
  - [x] Tests updated or executed unchanged for parity: `api/tests/api/companies.test.ts`, `api/tests/api/folders.test.ts`, `api/tests/api/initiatives.test.ts`, `api/tests/api/extended-objects.test.ts`, `api/tests/api/view-templates.test.ts`, `api/tests/api/gate-evaluation.test.ts` pass 134/134; the three initiative normalization/detail unit files pass 12/12. With cutover, root-fence, DOCX and collaboration security coverage, the final relevant API batch passes 10 files/195 tests.
  ...
  - [x] Lot gate: `make typecheck-api lint-api test-api-api SCOPE=tests/api/cluster-mesh-business-cutover.test.ts ENV=test-cluster-mesh-central-control-plane` passes typecheck, lint with 0 errors/208 baseline warnings, and 1 file/8 tests.
  ...
  - [x] Internal gates: C1 existing business tables/outbox remain canonical; C3/A5 use eight neutral injected router ports and one explicit product adapter; 38 exact root paths are registry-enrolled with no wildcard/null fence. Legacy source/import/mount and forbidden transport-import sweeps are empty; schema/control migrations are unchanged from `36a93f2b0`. Final cold boot reaches healthy, smoke passes 4/4, exact external probes return health 200, canonical business and DOCX paths 401, duplicate `/business` prefix 404, and the project is stopped afterward.
