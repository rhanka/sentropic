...
Status addendum 2026-06-01: **BR-42a split ratified and launched.** BR-42a0 (`feat/chat-server`, PR #201) extracts `@sentropic/chat-server@0.1.0`, migrates the current API onto it in `routes: 'app-contract'` mode, keeps `/tool-permissions` and non-chat SSE channels app-local, and provides the canonical generated-app route shape for BR-42a1. UAT passed on root `uat/42a0`; CI and first publish remain pending. BR-42a1 (`feat/build-app-cli`) must consume the published `@sentropic/chat-server@0.1.x` package before scaffolding generated chat apps.
...
- BR-42a0 `feat/chat-server` — **PR #201, UAT OK**. Extracts `@sentropic/chat-server` and migrates the
  current API onto it; CI and first npm publish pending before BR-42a1 starts.
...
|        |                                                  | wire+turn package over chat-core; migrate current API      | CI/publish pending   |                                |
|        |                                                  | in app-contract mode and expose canonical generated routes.|                      |                                |
+--------+--------------------------------------------------+------------------------------------------------------------+----------------------+--------------------------------+
