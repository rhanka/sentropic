...
Status addendum 2026-08-11: **BR-74 `fix/llm-gateway-compaction-usage` ACTIVE.** Routed Anthropic streams currently emit `message_start.usage.input_tokens=0` and defer provider usage to the terminal event, so Claude Code cannot observe context occupancy and can loop on `/compact`. BR-74 restores a request-derived, non-billing input-usage signal in the first Anthropic SSE frame, preserves output-only terminal usage and provider-reported settlement, adds wire/integration regressions, and requires real h2a exact-tarball UAT before any Sentropic PR. After merge-triggered gateway publication, h2a owns its published 0.94.x integration, regular-CI release, redeployment, and real Claude validation. Execution: root `BRANCH.md`; canonical contract amendment: `spec/SPEC_EVOL_LLM_MESH_GATEWAY_ROUTING.md`.
