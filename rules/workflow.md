...
- **Inventory check**: `validate-publishable-manifests` (`make check-publishable-manifests`) BLOCKs changed packages, publication-selected packages whose packed version is absent from npm, and explicit bootstrap targets; other public packages only WARN (known debt: `@sentropic/cowork-desktop` `file:` dependencies, owner unassigned). Registry lookup or snapshot failures are ERROR: re-run, not debt.
