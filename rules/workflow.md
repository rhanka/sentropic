...
- **Consumer qualification**: `make qualify-published-install PKG=@sentropic/<pkg>@<exact-version> [PEERS=<name>@<version>,...] ENV=<env>` installs and imports every Node entry point in a clean container (no repository mount, fresh npm cache, public registry). CI qualifies mcp-auth and cluster-mesh candidates (with guarded same-PR sibling archives) and their published versions: post-publication qualification runs when the publish receipt status is `published`, or on a re-run when it is `skipped` and the version is present on the registry.
- **Registry visibility**: registry waits are cache-busted (no-cache reads) with a ~180 s budget (18 x 10 s); a publish version conflict passes only when the registry holds the verified archive's sha512 (treated as already published), otherwise it fails; OIDC post-publication qualification checks that the SLSA provenance source commit equals the workflow commit.
