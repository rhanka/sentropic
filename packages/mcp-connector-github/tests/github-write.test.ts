/**
 * BR-72 Wave-2 Lot 1 — GitHub EXPERIMENTAL write-surface tests.
 *
 ...
 * `principal`); the 2 destructive tools (`delete_file`, `delete_repository`)
 ...
 * `invokeGithubWriteTool`); a properly-gated + idempotent mutation succeeds
...
import { githubManifest } from '../src/index.js';
import {
  getWriteToolFixture,
  githubWriteManifest,
  githubWriteToolsByName,
  invokeGithubWriteTool,
} from '../src/experimental.js';

const DESTRUCTIVE_TOOL_NAMES = new Set(['delete_file', 'delete_repository']);
...
describe('github connector EXPERIMENTAL write surface (BR-72 Wave-2 Lot 1)', () => {
  it('declares exactly the 7 planned write tools', () => {
    const names = githubWriteManifest.tools.map((t) => t.name).sort();
    expect(names).toEqual([...WRITE_TOOL_NAMES].sort());
    expect(githubWriteManifest.resources).toHaveLength(0);
  });

  it('every write tool declares mutatesExternalSystem:true and idempotency.required:true (scope principal)', () => {
    for (const tool of githubWriteManifest.tools) {
  ...
  it('destructive tools (delete_file, delete_repository) require human confirmation; others do not', () => {
    for (const tool of githubWriteManifest.tools) {
  ...
  it('the read-only Wave-1 root manifest stays mutation-free (regression guard)', () => {
    ...
    const readOnlyToolNames = githubManifest.tools.map((t) => t.name);
  ...
  it('assertMutationGate rejects an ungated mutation (no elicitation, no idempotency key)', () => {
    ...
    const capability = githubWriteToolsByName.get('create_issue');
    expect(capability).toBeDefined();
    const ctx = makeCtx(vi.fn(async () => 'SHOULD_NOT_BE_CALLED'));
    const envelope: AppToolInvocation = {
      capabilityRef: 'create_issue',
      input: { owner: 'sentropic-demo-user', repo: 'demo-repo-one', title: 'Synthetic issue' },
      ctx,
    };
  ...
  for (const capabilityRef of WRITE_TOOL_NAMES) {
    it(`invokeGithubWriteTool(${capabilityRef}) rejects an ungated invocation with ok:false + gate_failed, never touches getSecret`, async () => {
      const elicitations = new ElicitationManager();
      const auditEmit = vi.fn(async () => {});
      const getSecretSpy = vi.fn(async () => 'SHOULD_NOT_BE_CALLED');
      const ctx = makeCtx(getSecretSpy);
      const result = await invokeGithubWriteTool(
  ...
  for (const capabilityRef of WRITE_TOOL_NAMES) {
    it(`invokeGithubWriteTool(${capabilityRef}) succeeds once gated + idempotent, returning the synthetic fixture`, async () => {
      const elicitations = new ElicitationManager();
      const auditEmit = vi.fn(async () => {});
      const getSecretSpy = vi.fn(async () => 'SHOULD_NOT_BE_CALLED');
      const ctx = makeCtx(getSecretSpy);
  ...
  it('invokeGithubWriteTool returns ok:false unknown_tool for an unregistered capability', async () => {
    const elicitations = new ElicitationManager();
    const ctx = makeCtx(vi.fn(async () => 'SHOULD_NOT_BE_CALLED'));
    const result = await invokeGithubWriteTool(
