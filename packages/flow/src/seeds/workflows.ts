/**
 * @sentropic/flow — default workflow seed catalog (pure data).
 *
 ...
 *
 * Moved from `api/src/config/default-workflows.ts` at BR-26 Lot 5
 * (default-workflows + default-agents seeds slice). The legacy module
 * remains as a thin re-export to keep `api/` consumer imports stable
 * until Lot N-3 (BR26-EX3, BR26-Q5 lean: thin re-export until consumer
 * rebinding).
 *
 * Per spec/SPEC_EVOL_BR26_FLOW_FACADE.md §3 (façade design) + §4
 * (slice extraction order, Lot 5).
 */
