...
export async function createLoopbackServer(
  ...
  server = createServer((req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      // P0-2: Ignore non-callback requests (e.g. GET /favicon.ico)
      ...
      // P1-3: Prevent double-handling of replay callbacks
      if (handled) {
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end('<h1>Authentication completed</h1><p>Callback already consumed.</p>');
        return;
      }
