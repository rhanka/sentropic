/**
 * BR-72 benchmark proof — Linear `AppConnectorProviderAdapter`.
 *
...
export const linearConnectorAdapter: AppConnectorProviderAdapter = {
  appId: 'sentropic',
  connectorId: 'linear',
  ...
  async listCapabilities(_ctx: ConnectorTenantContext): Promise<AppCapability[]> {
    return [...linearManifest.resources, ...linearManifest.tools, ...linearManifest.prompts];
  },

  async readResource(req: AppResourceRead): Promise<AppResourceResult> {
    const auditId = nextAuditId('linear-resource');
  ...
  async invokeTool(req: AppToolInvocation): Promise<AppToolResult | DurableCallRef> {
    const auditId = nextAuditId('linear-tool');
    await req.ctx.audit.emit({ domain: 'linear', capabilityRef: req.capabilityRef });
  ...
  async validateSecrets(_ctx: ConnectorTenantContext): Promise<ConnectorSecretStatus> {
    ...
    return [
      { name: 'linearOAuthAccessToken', scope: 'principal', state: 'active' },
      { name: 'linearApiKey', scope: 'connector-instance', state: 'active' },
    ];
  },
};
