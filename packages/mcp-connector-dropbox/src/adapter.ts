/**
 * BR-72 Wave-1 benchmark proof — Dropbox `AppConnectorProviderAdapter`.
 *
...
export const dropboxConnectorAdapter: AppConnectorProviderAdapter = {
  appId: 'sentropic',
  connectorId: 'dropbox',
  ...
  async listCapabilities(_ctx: ConnectorTenantContext): Promise<AppCapability[]> {
    return [...dropboxManifest.resources, ...dropboxManifest.tools, ...dropboxManifest.prompts];
  },

  async readResource(req: AppResourceRead): Promise<AppResourceResult> {
    const auditId = nextAuditId('dropbox-resource');
  ...
  async invokeTool(req: AppToolInvocation): Promise<AppToolResult | DurableCallRef> {
    const auditId = nextAuditId('dropbox-tool');
    await req.ctx.audit.emit({ domain: 'dropbox', capabilityRef: req.capabilityRef });
