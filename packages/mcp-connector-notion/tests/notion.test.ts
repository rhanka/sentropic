/**
 * BR-72 Wave-1 benchmark proof — Notion connector adapter contract tests.
 *
...
import { notionConnectorAdapter } from '../src/adapter.js';
import { notionManifest } from '../src/manifest.js';
...
const resourceNames = notionManifest.resources.map((r) => r.name);
const toolNames = notionManifest.tools.map((t) => t.name);

describe('notion connector adapter (BR-72 read-only benchmark proof)', () => {
  it('declares only read-only capabilities (no mutatesExternalSystem: true anywhere)', () => {
    for (const resource of notionManifest.resources) {
    ...
    for (const tool of notionManifest.tools) {
    ...
    expect(notionManifest.prompts).toHaveLength(0);
  });

  it('listCapabilities returns exactly the declared resource + tool set', async () => {
    const tenantCtx = await notionConnectorAdapter.resolveTenant({
      principalSub: 'user-1',
      tenantRef: 'tenant-1',
      connectorInstanceId: 'connector-instance-1',
    });
    const capabilities = await notionConnectorAdapter.listCapabilities(tenantCtx);
  ...
  const resourceCases: Array<{ capabilityRef: string; uri: string }> = [
    { capabilityRef: 'retrieve_page', uri: 'notion://pages/demo-page-0000001' },
    { capabilityRef: 'retrieve_database', uri: 'notion://databases/demo-database-0000001' },
    { capabilityRef: 'retrieve_data_source', uri: 'notion://data-sources/demo-data-source-0000001' },
    { capabilityRef: 'retrieve_user', uri: 'notion://users/demo-user-0000001' },
  ];
  ...
  for (const { capabilityRef, uri } of resourceCases) {
    it(`readResource(${capabilityRef}) returns ok:true + fixture output + auditId, never touches getSecret`, async () => {
      ...
      const result = await notionConnectorAdapter.readResource({
        capabilityRef,
        input: { uri },
        ctx,
      });
  ...
  const toolCases = ['search', 'query_data_source', 'list_users', 'list_block_children'];
  for (const capabilityRef of toolCases) {
    it(`invokeTool(${capabilityRef}) returns ok:true + fixture output + auditId, never touches getSecret`, async () => {
      const getSecretSpy = vi.fn(async () => 'SHOULD_NOT_BE_CALLED');
      const ctx = makeCtx(getSecretSpy);
      const result = await notionConnectorAdapter.invokeTool({ capabilityRef, input: {}, ctx });
  ...
  it('validateSecrets discloses state only (never a secret value)', async () => {
    ...
    const statuses = await notionConnectorAdapter.validateSecrets(tenantCtx);
