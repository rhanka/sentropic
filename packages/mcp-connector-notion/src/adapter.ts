/**
 * BR-72 Wave-1 benchmark proof — Notion `AppConnectorProviderAdapter`.
 *
...
import { notionManifest } from './manifest.js';
...
export const notionConnectorAdapter: AppConnectorProviderAdapter = {
  appId: 'sentropic',
  connectorId: 'notion',
  ...
  async listCapabilities(_ctx: ConnectorTenantContext): Promise<AppCapability[]> {
    return [...notionManifest.resources, ...notionManifest.tools, ...notionManifest.prompts];
  },

  async readResource(req: AppResourceRead): Promise<AppResourceResult> {
    const auditId = nextAuditId('notion-resource');
  ...
  async invokeTool(req: AppToolInvocation): Promise<AppToolResult | DurableCallRef> {
    const auditId = nextAuditId('notion-tool');
    await req.ctx.audit.emit({ domain: 'notion', capabilityRef: req.capabilityRef });
