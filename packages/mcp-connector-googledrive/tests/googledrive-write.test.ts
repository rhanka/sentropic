/**
 * BR-72 Wave-2 Lot 2 — Google Drive EXPERIMENTAL write-surface tests.
 *
 ...
 * `principal`); the 2 destructive/authz-sensitive tools (`files.delete`,
 * `permissions.create`) declare `gates.requiresHumanConfirmation: true`,
 * others do not; an ungated mutation is REJECTED by `assertMutationGate` (both
 * directly and through `invokeGoogleDriveWriteTool`); a properly-gated +
 * idempotent mutation succeeds with a synthetic fixture result. No real
 * network. The read-only Wave-1 root (`../src/index.ts`) is asserted to
 * remain mutation-free (regression guard).
 */
...
import { googleDriveManifest } from '../src/index.js';
import {
  getWriteToolFixture,
  googleDriveWriteManifest,
  googleDriveWriteToolsByName,
  invokeGoogleDriveWriteTool,
} from '../src/experimental.js';

const DESTRUCTIVE_TOOL_NAMES = new Set(['files.delete', 'permissions.create']);
...
describe('googledrive connector EXPERIMENTAL write surface (BR-72 Wave-2 Lot 2)', () => {
  it('declares exactly the 6 planned write tools', () => {
    const names = googleDriveWriteManifest.tools.map((t) => t.name).sort();
    expect(names).toEqual([...WRITE_TOOL_NAMES].sort());
    expect(googleDriveWriteManifest.resources).toHaveLength(0);
  });

  it('every write tool declares mutatesExternalSystem:true and idempotency.required:true (scope principal)', () => {
    for (const tool of googleDriveWriteManifest.tools) {
      ...
      expect(['write', 'transaction']).toContain(tool.category);
      expect(tool.gates.requiresPrincipalGate).toBe(true);
      expect(tool.gates.requiresPolicy).toBe(true);
    }
  });

  it('destructive/authz-sensitive tools (files.delete, permissions.create) require human confirmation; others do not', () => {
    for (const tool of googleDriveWriteManifest.tools) {
    ...
    expect(googleDriveWriteToolsByName.get('files.delete')?.mutability).toBe('delete');
    expect(googleDriveWriteToolsByName.get('permissions.create')?.mutability).toBe('state-transition');
  });

  it('the read-only Wave-1 root manifest stays mutation-free (regression guard)', () => {
    for (const resource of googleDriveManifest.resources) {
    ...
    const readOnlyToolNames = googleDriveManifest.tools.map((t) => t.name);
  ...
  it('assertMutationGate rejects an ungated mutation (no elicitation, no idempotency key)', () => {
    ...
    const capability = googleDriveWriteToolsByName.get('files.create');
    expect(capability).toBeDefined();
    const ctx = makeCtx(vi.fn(async () => 'SHOULD_NOT_BE_CALLED'));
    const envelope: AppToolInvocation = {
      capabilityRef: 'files.create',
      input: { name: 'demo.txt' },
      ctx,
    };
  ...
  for (const capabilityRef of WRITE_TOOL_NAMES) {
    it(`invokeGoogleDriveWriteTool(${capabilityRef}) rejects an ungated invocation with ok:false + gate_failed, never touches getSecret`, async () => {
      const elicitations = new ElicitationManager();
      const auditEmit = vi.fn(async () => {});
      const getSecretSpy = vi.fn(async () => 'SHOULD_NOT_BE_CALLED');
      const ctx = makeCtx(getSecretSpy);
      const result = await invokeGoogleDriveWriteTool(
  ...
  for (const capabilityRef of WRITE_TOOL_NAMES) {
    it(`invokeGoogleDriveWriteTool(${capabilityRef}) succeeds once gated + idempotent, returning the synthetic fixture`, async () => {
      const elicitations = new ElicitationManager();
      const auditEmit = vi.fn(async () => {});
      const getSecretSpy = vi.fn(async () => 'SHOULD_NOT_BE_CALLED');
      const ctx = makeCtx(getSecretSpy);
  ...
  it('invokeGoogleDriveWriteTool returns ok:false unknown_tool for an unregistered capability', async () => {
    const elicitations = new ElicitationManager();
    const ctx = makeCtx(vi.fn(async () => 'SHOULD_NOT_BE_CALLED'));
    const result = await invokeGoogleDriveWriteTool(
