 ...
 * mode/TTL. Per-capability `gates.requiresElicitation` remains authoritative; a
 * policy here never relaxes a stricter per-capability gate.
 */
export type ElicitationPolicy = {
  capabilityRef: string; // capability name within the manifest
  mode: 'form' | 'confirm' | 'consent' | 'url' | 'credential';
  ttlSeconds: number;
  required: boolean;
};
