/**
 * BR14b Lot 14b — chat history projection helpers.
 *
 * Moved verbatim from `api/src/services/chat-session-history.ts` because
 * these helpers are pure functions (no IO, no DB, no service dependency)
 * used by the 3 read-only composers `getSessionBootstrap`,
 * `getSessionHistory`, and `getMessageRuntimeDetails`. Migrating them into
 * chat-core is a prerequisite for migrating the composers themselves into
 * `ChatRuntime` (Lot 14b Commit 2).
 *
 * Behavior preservation is the absolute contract: each helper is a
 * line-for-line move of the body that lived in `chat-session-history.ts`.
 * The api-side file is replaced by a re-export shim so external callers
 * (test fixtures, etc.) keep their import path stable.
 */
