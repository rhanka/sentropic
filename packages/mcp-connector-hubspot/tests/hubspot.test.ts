/**
 * BR-72 read-only benchmark proof — HubSpot connector adapter contract tests.
 *
...
import { hubspotConnectorAdapter } from '../src/adapter.js';
import { hubspotManifest } from '../src/manifest.js';
...
function makeCtx(getSecretSpy: ReturnType<typeof vi.fn>): StpConnectorContext {
  return {
    ...
    principal: {
      sub: 'user-1',
      claims: {},
      scopes: ['hubspot.contacts.read', 'hubspot.companies.read', 'hubspot.deals.read', 'hubspot.crm.read'],
      tenantRef: 'tenant-1',
...
const resourceNames = hubspotManifest.resources.map((r) => r.name);
const toolNames = hubspotManifest.tools.map((t) => t.name);

describe('hubspot connector adapter (BR-72 read-only benchmark proof)', () => {
  it('declares only read-only capabilities (no mutatesExternalSystem: true anywhere)', () => {
    for (const resource of hubspotManifest.resources) {
    ...
    expect(hubspotManifest.prompts).toHaveLength(0);
  });

  it('listCapabilities returns exactly the declared resource + tool set', async () => {
    const tenantCtx = await hubspotConnectorAdapter.resolveTenant({
      principalSub: 'user-1',
      tenantRef: 'tenant-1',
      connectorInstanceId: 'connector-instance-1',
    });
    const capabilities = await hubspotConnectorAdapter.listCapabilities(tenantCtx);
  ...
  const resourceCases: Array<{ capabilityRef: string; uri: string }> = [
    { capabilityRef: 'get_user_details', uri: 'hubspot://user' },
    { capabilityRef: 'get_contact', uri: 'hubspot://contacts/3000001' },
    { capabilityRef: 'get_company', uri: 'hubspot://companies/4000001' },
    { capabilityRef: 'get_deal', uri: 'hubspot://deals/5000001' },
  ];
  ...
  for (const { capabilityRef, uri } of resourceCases) {
    it(`readResource(${capabilityRef}) returns ok:true + fixture output + auditId, never touches getSecret`, async () => {
      ...
      const result = await hubspotConnectorAdapter.readResource({
        capabilityRef,
        input: { uri },
        ctx,
      });
  ...
  const toolCases = ['search_crm_objects', 'list_properties', 'search_owners', 'get_campaign_analytics'];
  for (const capabilityRef of toolCases) {
    it(`invokeTool(${capabilityRef}) returns ok:true + fixture output + auditId, never touches getSecret`, async () => {
      const getSecretSpy = vi.fn(async () => 'SHOULD_NOT_BE_CALLED');
      const ctx = makeCtx(getSecretSpy);
      const result = await hubspotConnectorAdapter.invokeTool({ capabilityRef, input: {}, ctx });
  ...
  it('validateSecrets discloses state only (never a secret value)', async () => {
    ...
    const statuses = await hubspotConnectorAdapter.validateSecrets(tenantCtx);
