/**
 * BR-72 read-only benchmark proof — Google Calendar `AppConnectorProviderAdapter`.
 *
...
import { googlecalendarManifest } from './manifest.js';
...
export const googlecalendarConnectorAdapter: AppConnectorProviderAdapter = {
  appId: 'sentropic',
  ...
  async listCapabilities(_ctx: ConnectorTenantContext): Promise<AppCapability[]> {
    return [...googlecalendarManifest.resources, ...googlecalendarManifest.tools, ...googlecalendarManifest.prompts];
  },

  async readResource(req: AppResourceRead): Promise<AppResourceResult> {
    const auditId = nextAuditId('googlecalendar-resource');
  ...
  async invokeTool(req: AppToolInvocation): Promise<AppToolResult | DurableCallRef> {
    const auditId = nextAuditId('googlecalendar-tool');
    await req.ctx.audit.emit({ domain: 'googlecalendar', capabilityRef: req.capabilityRef });
