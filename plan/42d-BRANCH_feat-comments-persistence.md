...
- [x] **Lot 5 — AI re-route** (M-AI-READ / M-AI-WRITE / M-AUTO)
  - [x] `api/src/services/tool-service.ts` read path: re-route `listCommentThreadsForContexts` grouping onto the shared `store.listByTarget` (per-context loop; host re-applies the `(contextType,contextId)` pair + threadId filter, global `createdAt ASC, id ASC` ordering, `limit ?? 200` slice; host mapper `buildThreadSummariesFromComments` reproduces the live shape `assignedTo`/`status open|closed`/derived `createdBy/createdAt/updatedAt` from the rows — uses `listByTarget` NOT `listThreadSummaries` because the package summary OMITS `createdBy/createdAt/updatedAt`, exactly the SPEC §1bis #2 GAP-B resolution); KEEP the `users` join app-local.
  - [x] `api/src/services/tool-service.ts` write path: re-route `resolveCommentActions` close→`setState`, reassign→`assign`, trace-note→`add` (all EMIT-FREE); emit explicit `{action, key}` via the sink: close→`{closed, thread_id}`, reassign→`{reassigned, thread_id}`, trace-note→`{created, comment_id}`; KEEP AI gating (`hasWorkspaceRole`/`ensureWorkspaceMember`/allowed-context) app-local. The trace-note inherits the post-cascade assignee (passed to `add`) + a `setState('resolved')` emit-free reconcile when `latestStatus==='closed'` (reproduces the live `status: latestStatus` insert).
  - [x] `api/src/services/queue-manager.ts` + tool-service auto path (M-AUTO): re-route both auto-field inserts to `store.add` (mints fresh thread + `status:'open'`); emit `{created, comment_id}` (origin `auto`) via the SAME shared store+sink instance (out-of-request lifecycle, spec DEC-4). The queue path preserves the live `auto_generation:<comment_id>` self-referential provenance via a post-add provenance-only reconcile.
  - [x] `api/src/services/context-comments.ts`: NO edit needed — the summary INPUT already flows from the port: `chat-service.ts:4401` calls the re-routed `listCommentThreadsForContexts` (port + host mapper) and feeds `list.threads` into `generateCommentResolutionProposal` (`:4410`); the prompt + proposal stay app-local.
  - [x] DELETE the 2 remaining `notifyCommentEvent` copies (`tool-service.ts`, `queue-manager.ts`) IN THIS SAME lot. After this lot the sink is the SOLE comment NOTIFY emitter (0-legacy — grep `NOTIFY comment_events|notifyCommentEvent` over `api/src` returns only `pg-notify-comment-event-sink.ts`).
  - [x] Lot gate:
    - [x] `make typecheck-api` clean + `make lint-api ENV=test-feat-comments-persistence` (0 errors, no NEW warnings on the edited files).
