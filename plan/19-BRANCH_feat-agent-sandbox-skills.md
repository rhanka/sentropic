...
- [x] `attention` BR19-UAT2 — User reported during BR-19 UAT prep that DOCX generation makes the UI enter a loop, and the same symptom is visible in production. Reproduced on isolated env `test-feat-agent-sandbox-skills` from Ellio organization context with session `a179bde6-6119-416c-9d29-1ecf76694af5`: stream initially hit 500 events without `done`, `document_generate` started 3 times, and tool errors were `document_generate: entityId is required` then `document_generate: entityType must be "initiative" | "folder"`. Root cause was not BR14a stream projection: organization context exposed `document_generate`, but the executor only accepts `initiative` or `folder`, causing the model to retry tool calls. Fix retargets single-folder organization context to the linked folder, hardens the document-generation system prompt, and adds the `document` alias to DOCX freeform helpers/host bridge after second repro `66ed8396-f3ac-43b5-946c-ac695cece24b` failed with `ReferenceError: document is not defined`. Final evidence: session `902b11f8-ab27-488e-93f4-93968d15b125`, assistant `a9b934ff-dfc7-4780-8458-fef9d8c29dc8`, chat job `1c5be489-e429-4bd9-b42a-90db29d304d8`, document job `b3a63608-d99b-4599-8786-7f8be766cbbc`, `documentGenerateStarts=1`, `toolErrors=0`, `doneEvents=1`, `documentCards=1`, UI reload plus explicit session selection shows final download card, and API download returns `200` with DOCX MIME type, 8925 bytes, first bytes `50 4b 03 04`. Root UAT remains pending under BR19-UAT1.
