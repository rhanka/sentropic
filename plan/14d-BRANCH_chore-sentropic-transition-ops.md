...
- Scaleway Container Serverless names, registry image names, secrets, workflows, dashboards, metadata — DONE (BR-37d): legacy serverless container `top-ai-ideas-api` + managed DB `top-ai-ideas-db` decommissioned; legacy deploy machinery removed from `ci.yml`/`Makefile` (only `deploy-k8s` remains). RESIDUAL (name-only): the SCW source image tag `top-ai-ideas-ui` (`SOURCE_UI_IMAGE_NAME` in `ci.yml`) and the bucket `top-ai-ideas-docs` — fold into the BR-14e residual-name report.
