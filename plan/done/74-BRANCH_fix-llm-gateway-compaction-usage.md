...
- [x] Restore the Anthropic streaming usage contract required by Claude Code
  compaction without buffering a routed response or changing financial usage.
- [x] Prove the exact package in real h2a integration before opening a PR.
...
- [x] Mono-branch Sentropic implementation; h2a owns its isolated integration
  lane and all h2a source, release, and deployment changes.
- [x] Objective loop `loop-gw-compaction-release-20260811` remains active
  until Sentropic npm publication, h2a 0.94.x npm publication, redeployment,
  and the real Claude smoke all pass.
- [x] `BR74-F1` blocking: h2a must confirm the exact compaction contract and
  pre-PR UAT recipe.
...
- [x] **Lot 0 — Contract, branch, specification, and Track**
  - [x] Create the isolated worktree and pass `harness check branch`.
  - [x] Archive the merged BR-73 plan and register BR-74 in `PLAN.md`.
  - [x] Amend the routing evolution spec with the Anthropic usage invariant.
  - [x] Import this plan into Track and validate the append-only sidecar.
  - [x] Reconcile the exact contract and UAT recipe with the live h2a owner.

- [x] **Lot 1 — Red contract and integration tests**
  - [x] Assert `message_start.usage.input_tokens` is request-derived and
    non-zero for text/system/tool context before provider completion.
  - [x] Assert terminal `message_delta.usage` contains output usage only.
  - [x] Assert OpenAI stream usage, settlement usage, SSE order, tools,
    reasoning, and first-frame streaming remain unchanged.
  - [x] Cover long context, Unicode, tools, and binary/image payload handling
    so estimation cannot scale with embedded base64 bytes.

- [x] **Lot 2 — Minimal gateway fix and package candidate**
  - [x] Compute bounded request-derived Anthropic input usage before streaming.
  - [x] Pass it only to the Anthropic canonical encoder; keep provider-reported
    usage authoritative for metering and settlement.
  - [x] Bump only `@sentropic/llm-gateway` to a registry-unique patch.
  - [x] Pass focused/full tests, typecheck, build, pack, scope, and branch gates.
  - [x] Produce the exact tarball, commit SHA, version, and SHA-256 evidence.

- [ ] **Lot 3 — Mandatory h2a UAT before PR**
  - [x] h2a installs the exact tarball in an isolated worktree.
  - [x] Direct SSE integration proves first-frame input usage and output-only
    terminal usage without response buffering.
  - [x] Real Claude Code through the enrolled gateway completes two successive
  ...
  - [x] Fix every blocking finding and repeat the affected gates and UAT.

- [ ] **Lot 4 — Review, PR, publication, and downstream release**
  - [ ] Obtain two exact-head independent reviews and reconcile all P0/P1.
  - [x] Open the Sentropic PR only after Lot 3 PASS; obtain green PR CI and merge.
  - [ ] Archive this plan, remove root `BRANCH.md`, and run branch lifecycle.
  - [x] Verify the new gateway version is visible on npmjs.org.
...
- [x] No Sentropic PR exists before exact-tarball h2a UAT passes.
- [x] No response buffering, extra provider call, or changed billing occurs.
