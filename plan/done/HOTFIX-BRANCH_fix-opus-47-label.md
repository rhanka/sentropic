...
- **BR19-N4 — Vocabulary cleanup per glossary (2026-05-22)**
  - Context: prior Wave D Step 1.A/1.B commits (`b8fc1c1d`, `c4dea8ed`, `7cc55ff2`) and the in-flight Step 1.C scaffolding introduced the term `curriculum` to label the LLM-facing skill-instructions text returned by `action=upskill`. The project glossary (`spec/SPEC_STUDY_SKILLS_TOOLS_VS_AGENT_MARKETPLACE.md §1`) restricts canonical vocabulary to `Tool` / `Skill` / `Agent` / `Plugin`; `curriculum` is not in that table and user has rejected the term twice. Root cause analysis (session limit interrupted the 2026-05-19 conversation before the rename was applied) traced in handover; details out of scope here.
  - Decision: replace `curriculum` with `skill instructions` (or inline section under `SKILL.md`) across `packages/skills/src/bundles/foundation/document_generate/**` and `BRANCH.md`. The 221 lines of DOCX/PPTX helper API previously staged in `curriculum/{docx,pptx}.md` are inlined into the bundle's `SKILL.md` body as `# DOCX sandbox helpers (API surface)` and `# PPTX sandbox helpers (API surface)` sections, matching the Anthropic SKILL.md single-document convention.
  - Closes: dependency for BR19-D6.
  - Anchor: glossary spec extended with §1.1 "Banned synonyms" table (curriculum / upskill / guide / handbook / recipe / workflow / bot → canonical Tool / Skill / Agent / Plugin / Flow). Documented in `spec/SPEC_STUDY_SKILLS_TOOLS_VS_AGENT_MARKETPLACE.md` per BR19-EX8. No Makefile target, no CI lint job — enforcement is by review (deliberate choice: a lint target on top-level vocabulary is over-engineering; the spec + this BRANCH.md drift incident record are the anchor).

- **BR19-EX8 — `spec/SPEC_STUDY_SKILLS_TOOLS_VS_AGENT_MARKETPLACE.md` (additive only, BR19-N4)**
  - Reason: BR19-N4 anchor — the canonical glossary on `main` is `spec/SPEC_STUDY_SKILLS_TOOLS_VS_AGENT_MARKETPLACE.md §1` (Tool / Skill / Agent / Plugin / Marketplace). Path is NOT in BR19 Allowed Paths (the `spec/` allowances are scoped to `SPEC_EVOL_BR19_SKILLS.md` and `SPEC_VOL_AGENT_SANDBOX_SKILLS.md`). The vocab anti-recurrence anchor belongs in the existing glossary, not in a new file. Mutualized into BR19 instead of a separate `chore/vocab-policy` branch because (a) BR19 is the branch where the drift occurred and was caught, (b) it shrinks the loop between "incident" and "anchor", (c) precedent set by BR19-EX5/EX6.
  - Impact: single additive sub-section §1.1 "Banned synonyms" (~30 lines, one table + a rationale paragraph + a drift-incident log). No deletions, no restructuring. Other sections of the document untouched.
  - Rollback: revert the §1.1 sub-section; canonical §1 table remains intact.

- **BR19-EX7 — `api/tests/unit/**` legacy test cleanup (additive deletions only, BR19-D6)**
  - Reason: BR19-D6 deletes `api/src/services/{docx,pptx}-freeform-skill.ts`. Existing unit tests in `api/tests/unit/{docx,pptx}-freeform-helpers.test.ts` import the soon-to-be-deleted functions, and `api/tests/unit/chat-service-document-generate{,-pptx}.test.ts` assert on the `mode: 'upskill'` chat-service branch that is being removed. Tests must be aligned with the legacy removal in the same commit (no dual-paths rule).
  - Impact: surgical removals only — drop the `describe('upskill skill content')` block in `docx-freeform-helpers.test.ts` (~10L), the `'should return compact upskill guidance'` test in `pptx-freeform-helpers.test.ts` (~5L), and the `'should return upskill content'` / `'returns PPTX upskill content'` test cases in the two chat-service tests (~110L combined). No new tests, no helpers refactor. Helper tests for `doc`/`h`/`p`/`list`/`table`/sandbox execution remain unchanged.
  - Rollback: if the upskill removal is reverted (BR19-D6 rollback), restore the deleted blocks alongside.
...
- **BR19-D6 — `action=upskill` mode removed (Wave D Step 1.C, 2026-05-22)**
  - Decision: drop the legacy `action=upskill` mode from `document_generate` entirely instead of porting it to a sandbox handler. The LLM discovers the helper API via `search_skills` (Lot 4) plus native loading of the bundle's `SKILL.md` body (which now inlines the `# DOCX sandbox helpers` / `# PPTX sandbox helpers` sections — see BR19-N4).
  - Rationale: `action=upskill` is a pre-skill-loading legacy hack — a tool-call indirection that fetches an instruction string the LLM then injects before re-invoking `action=generate`. SOTA per `spec/SPEC_STUDY_SKILLS_TOOLS_VS_AGENT_MARKETPLACE.md §1` (Anthropic/Claude Code) makes this redundant: the SKILL.md body IS the skill, discoverable via description-match. Master rule "no legacy fallback / zero dual paths" also forbids keeping the deprecated path alive after the SOTA replacement is in place.
  - Impact:
    - `api/src/services/tools.ts:875` `enum: ['upskill', 'generate']` → `['generate']` + tool description rewritten to point at SKILL.md.
    - `api/src/services/chat-service.ts:4741-4760` upskill branch removed; rejection guard added (`action must be "generate"`). System prompt at L2759 updated to remove the "call upskill first" instruction and point to `search_skills`.
    - `packages/skills/src/bundles/foundation/document_generate/handler.ts:120-122` deferred-`upskill` guard removed (now redundant with the `action !== 'generate'` rejection).
    - `packages/skills/src/bundles/foundation/document_generate/SKILL.md` frontmatter schema + tool description aligned (`enum: [generate]` + reference to inline helper sections).
    - DELETE `api/src/services/docx-freeform-skill.ts` (152L) and `api/src/services/pptx-freeform-skill.ts` (92L).
    - Tests aligned per BR19-EX7: removed legacy upskill test cases in `api/tests/unit/{docx,pptx}-freeform-helpers.test.ts` and `api/tests/unit/chat-service-document-generate{,-pptx}.test.ts`.
  - Closes Wave D Step 1.C upskill sub-path with status `removed-not-ported`. Template DOCX sub-path remains deferred to BR19-D3 closing rebind per BR19-N3.
...
- **BR19-Q7 — `document_generate` port — sandbox-invocation pattern ambiguity (2026-05-19, BLOCKING Wave D step 1)**
  ...
  - Evidence gathered:
    1. `api/src/services/docx-freeform-skill.ts` is a TEXT skill-instructions string (`getDocxFreeformSkill(): string`) returned by chat-service when `args.action === 'upskill' && format === 'docx'`. It is NOT executable sandbox code and has no callable handler shape.
...
- [ ] **Lot 5 — Migrate `tools.ts` to skill bundles (waves)**
  ...
  - [ ] Migrate **wave D** (sandbox-backed skills): `document_generate` (uses sandbox, ports the docx-freeform skill), `batch_create_organizations`, `task_dispatch`.
    ...
    - [x] Step 1.C — `action=upskill` mode removed per BR19-D6 (SOTA = `search_skills` + native SKILL.md loading); helper API inlined into `SKILL.md` per BR19-N4. Legacy services `api/src/services/{docx,pptx}-freeform-skill.ts` deleted; `api/src/services/tools.ts` + `chat-service.ts` upskill dispatch removed; corresponding test cases pruned per BR19-EX7. Template DOCX sub-path (`action=generate && format=docx && templateId`) still routes through the BR19-N3 `caller.templateRenderer` deferred-binding error; the actual binding moves to BR19-D3 closing rebind.
