...
- **Conditional Paths (allowed only with explicit exception)**:
  - `Makefile` (`BR37d-EX1` removed dead legacy serverless targets; `BR37d-EX4` retargets `db-backup-prod` to the k8s prod DB)
  - `.github/workflows/ci.yml` (`BR37d-EX3` removed legacy deploy jobs; `BR37d-EX4` retargets the `test-smoke-restore` backup step to k8s)
...
- **BR37d-EX4** (status: `used` 2026-05-29): retargeted the `test-smoke-restore` CI migration test off the deleted managed prod DB. Modifies `Makefile` target `db-backup-prod` (now dumps the **k8s in-cluster prod postgres** via `kubectl exec` + `KUBECONFIG`, with a `test -s` gate so an empty dump fails loudly — the silent failure that masked BR37d-FL4) and the `.github/workflows/ci.yml` `test-smoke-restore` "Backup production database" step (decode `KUBECONFIG_B64` → `make db-backup-prod KUBECONFIG=…`; drops the now-dead `DATABASE_URL_PROD`/`DB_SSL_CA_PEM_B64` secret env). `db-restore` (local restore + migrations + prod-mode smoke) is unchanged. Goes beyond BR37d-EX1's append-only clause (modifies an existing target), justified by the operator: the pre-merge migration test must run against the real prod data, which now lives in k8s. Rollback: revert both hunks. Operator follow-up: the GH secrets `DATABASE_URL_PROD`/`DB_SSL_CA_PEM_B64` are now unused and can be deleted.
- **BR37d-FL4** (severity: `blocker`, status: `resolved` 2026-05-29): first PR CI (run 26613953946) red — `test-smoke-restore` failed `pg_restore: input file is too short (read 0)`. Root cause: deleting the managed prod DB (Lot 3) left the CI migration test backing up from it via `DATABASE_URL_PROD` → empty dump → restore fails (the old `db-backup-prod` swallowed the `pg_dump` failure). Fixed via BR37d-EX4 (retarget to k8s prod). Verified live: `make db-backup-prod KUBECONFIG=…` produced a 32 MB valid CUSTOM archive (Dumped from 17.10, 422 TOC entries); prod + local both `postgres:17-alpine` (no version skew). The concurrent `test-e2e (group-e)` failure is the documented AI flaky-accepted signature; rerun on the same commit.
