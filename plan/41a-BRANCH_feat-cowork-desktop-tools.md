...
- [ ] **Lot 0 — Baseline & constraints**
  ...
  - [ ] Create/confirm isolated worktree `tmp/feat-cowork-desktop-tools`; verify branch.
...
- [ ] **Lot 2 — `@sentropic/cowork-bridge` + chrome-ext refactor**
  ...
  - [ ] Re-point `ui/src/lib/core` consumers and refactor `ui/chrome-ext` (`extension-auth.ts`,
        tool/permission types) to consume the bridge; Chrome implements `StorageAdapter` with
        `chrome.storage`.
  - [ ] Lot gate:
    - [ ] `make typecheck-ui` + `make lint-ui`
    - [ ] **UI tests**: bridge unit tests (`packages/cowork-bridge/tests/*.spec.ts` — token math,
          adapter seam, protocol types); update any moved-module tests under `ui/`.
    - [ ] **Chrome-ext non-regression**: build the extension (`make build-ext-chrome`) and run its
          test suite; confirm green (the extension already ships).
...
- [ ] **Lot 3 — Backend device-code enrollment + device registry**
  ...
  - [ ] Extend the presence registry for `source: "desktop_cowork"` (BR41a-Q1 default: extend
        `tab-registry`); keepalive/eviction reused.
  - [ ] Lot gate:
    - [ ] `make typecheck-api` + `make lint-api`
    - [ ] **API tests**: add `api/tests/api/auth-device-code.spec.ts` (code issue, poll pending/
          approved/expired/slow-down, single-use); update registry tests for the new source.
    - [ ] Sub-lot gate: `make test-api ENV=test-cowork-desktop-tools`

- [ ] **Lot 4 — Desktop tools (eyes + hands) + consent**
  - [ ] Implement `screen_capture` (capture lib) and `input_action` (`click`/`type`/`scroll`/`key`)
        as `ToolExecutor`s in the binary; pure-JS/prebuilt libs where possible.
...
- [ ] **Lot 5 — Portable Windows binary packaging**
  - [ ] `esbuild` bundle of the binary entry; package via Node SEA (fallback pkg; fallback
        folder-zip) cross-built from Docker/Linux (`BR41a-EX1`/`EX2`).
  - [ ] New `make build-desktop` / `make package-desktop-windows` target; output `ui/static/desktop/`
        + a download metadata endpoint analogous to the chrome-ext download.
...
- [ ] **Lot N-2 — UAT**
  - [ ] Chrome plugin (non-regression): connect, run `tab_read`/`tab_action`, chat streaming — must
        behave exactly as before the bridge refactor.
  - [ ] Cowork binary (Windows): unzip, launch, device-code pairing (`PAIR-XXXX`), appears as a chat
        target, grant consent, run `screen_capture` + `input_action` from the Sentropic chat.
  - [ ] Web app (non-regression): chat + existing tab targeting unaffected.

- [ ] **Lot N-1 — Docs consolidation**
  - [ ] Update `spec/SPEC_COWORK.md` to match the delivered behavior; cross-link from
        `spec/SPEC_CHROME_PLUGIN.md` where the bridge is shared.
