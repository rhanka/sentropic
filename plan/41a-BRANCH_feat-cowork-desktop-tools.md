...
> **CLOSED — merged via PR #192 (merge commit) on 2026-05-31.** This file is the executed record,
> archived from the worktree `BRANCH.md` at close (the working `BRANCH.md` was removed for merge).
> Delivered: `@sentropic/cowork-bridge` (extracted + consumed in-situ by web/chrome-ext/vscode-ext),
> backend device-code enrollment + `desktop_cowork` registry + F1 tab-tool gate, `@sentropic/cowork-desktop`
> (device-code client, eyes/hands executors, per-tool consent), single signable Windows `.exe` via
> `@yao-pkg/pkg` (unsigned for now), admin-only Settings download card + release/prerelease channel,
> and npm publish plumbing (OIDC + bootstrap) for both packages.
> Deferred follow-ups: exe code-signing (resold OV cert later → `jsign`), real Windows binary UAT,
> `spec/SPEC_COWORK.md` docs sync. AI flake (`chat-tools` timeout, allowlisted) accepted with user
> sign-off (same-commit rerun green). Post-merge: `workflow_dispatch bootstrap_publish_target=cowork-bridge`
> then `=cowork-desktop` → first publishes → attach `cowork-desktop` npm trusted publisher → unlock BR-41b.
...
- [~] **Lot N-2 — UAT** (non-regression DONE in CI; real binary UAT DEFERRED post-merge by user)
  - [x] Chrome plugin (non-regression): covered by CI e2e `03-chat-chrome-extension*` (green) + `build-ext-chrome`.
  - [ ] **DEFERRED (post-merge, Windows host)** — Cowork binary: unzip, launch, device-code pairing,
        appears as chat target, grant consent, run `screen_capture` + `input_action` from the chat.
        User explicitly will NOT UAT the binary on this branch; done on a Windows machine after deploy
        (download via the admin-only Settings card / prerelease channel).
  - [x] Web app (non-regression): CI e2e `06-settings` (download cards admin-only) + full suite green.

- [~] **Lot N-1 — Docs consolidation** (DEFERRED, minor)
  - [ ] **DEFERRED** — sync `spec/SPEC_COWORK.md` to delivered behavior (device-code endpoints, desktop
        package, pkg packaging, prerelease channel, signing decision). Tracked as follow-up; the
        Feedback Loop above + plan file are the current authoritative record.

- [x] **Lot N — Final validation**
  - [x] Typecheck & lint (ui + api + packages) green (CI).
  - [x] UI / API / E2E green on CI (commit `39ea601a`, all e2e shards `success`); e2e shard flakiness
        is provider/contention nondeterminism (allowlisted AI specs), confirmed by same-commit reruns.
  - [x] New packages `@sentropic/cowork-bridge` + `@sentropic/cowork-desktop` at `0.1.0`
        (`enforce-package-bump` green for new packages).
  - [x] Final gate: PR #192; AI flake signed off by user; CI green; archive plan + remove `BRANCH.md` →
        merge via merge commit. Post-merge: bootstrap-publish bridge+desktop → attach desktop trusted
        publisher → unlock BR-41b.
