...
Before Focus live-write may produce an owner SIGNATURE, the co-specified gate MUST define and enforce: owner/workspace authentication using the own-principal model and the PR #416 API tenancy resolver in direct/strict FAIL-CLOSED product mode, never its default `shadow` mode; authorization for the decision and workspace; ATTESTER-vs-RELAYER provenance, with the owner as attester and any transport as relayer only; a pinned Track ingest contract version; an idempotency key and duplicate-result semantics; and read-back confirmation of the persisted attestation. Until every item is specified and satisfied, a Track outcome is **NOT** an owner signature.

## 3. Placement and sequencing

Orientation #3 is remote: Sentropic owns durable workspace, exposure, agents-surface, Focus, and Track primitives; h2a remains holder of remote CLI presence and submits integrations into those surfaces. **Part 3 is the priority-first design dependency because it becomes the owner's signature mechanism.** #502 is explicitly pre-signature: neither its merge nor any artifact is an owner signature. Until the co-specified `FocusLiveSession` owner-authenticated signature gate is satisfied and its persisted attestation is confirmed by read-back, no owner signature exists and no build lane opens. Part 1 remains near-term in design only, while Part 2 waits for the BR-39l api/h2a join and listing endpoint; its durable `agentRef`/cross-conversation collapse additionally waits for a published h2a identity repair.

The currently deferred Focus live-write driver (`FocusLiveSession` / the live write path) together with its security gate is a co-specification dependency, not an authorized build item in this design: it must close the reuse/composition gap so the existing Focus dossier can record a verified owner signature in Track, **not** create a new signature UI. Dependencies remain: #502 for the fused surface; #416 for the internal DB-backed tenancy resolver, which must run in direct/strict FAIL-CLOSED product mode; #439 only for the async mcp-platform `TenantResolver` seam; #492 only for best-effort / fail-open connector-grant teardown, never as a fail-closed durable-teardown guarantee; h2a PR #152 for the invariant publisher; Focus/Track for decisions. Any security requirement for fail-closed durable teardown remains CLOSED until a published contract is co-specified and pinned. No implementation starts before both runtime 0.90.1 and an actual owner signature recorded and read back through the satisfied Part-3 gate; there is no pre-Focus exception. Only after both gates open may Codex TERRA start the approved app/chat/agents lane sequence.

## 4. LIB/INTEGRATION split and acceptance grid

| Part | LIB — Sentropic owns | INTEGRATION — h2a owns |
|---|---|---|
| 1. Code workspaces | Enrollment over `workspaces` + membership + the PR #416 internal DB-backed resolver in direct/strict FAIL-CLOSED product mode; own the unique `H2AWorkspaceRef.id -> workspaces.id` binding; publish the production authorization port through future P1-A before cross-repo use | Submit code-space enrollment only after the authorization-port gate opens; retain and send only the durable `H2AWorkspaceRef.id` across the boundary |
...
- **C — Order and security:** remote presence and operations obey descriptor-specific timestamps plus envelope `asOf`, feed-computed liveness/state without inferring connectivity from `idle`, exact MCP-versus-heartbeat provenance, HOLDER/VIEW authority, non-authoritative instance-only display capabilities, and own-principal workspace/tenant authorization through the PR #416 resolver's direct/strict FAIL-CLOSED product mode. #492's capture-before, tombstone/outbox-during, and revoke-after-commit teardown is best-effort / fail-open and supplies no fail-closed durable-teardown or no-orphan guarantee; any security gate needing one remains closed until a published fail-closed contract is co-specified and pinned. Durable `agentRef` and cross-conversation collapse remain closed until the future h2a identity repair is published and pinned. A Part-3 outcome is not a signature without authenticated owner attestation, authorization, provenance, versioned idempotent ingest, and confirmed read-back; neither a merge nor an artifact substitutes for that signature. Runtime 0.90.1 plus the actual owner signature gates every lane.
...
- **Part-3 gate — authentication:** specify owner/workspace authentication through own-principal and the PR #416 API tenancy resolver in direct/strict FAIL-CLOSED product mode.
