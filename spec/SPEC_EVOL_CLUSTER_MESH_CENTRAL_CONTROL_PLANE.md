...
- **Lot C auth isolation is preserved.** `/gateway` maps only to gateway root and pulls no session-auth code or declarations. `/gateway/auth-hono` maps solely to `@sentropic/llm-gateway/auth-hono`; service mode uses published `@sentropic/mcp-auth@0.2.0` via `./hono`, while session mode alone acquires optional `@sentropic/auth-hono`. Missing session auth refuses that mode. Public-tarball service-auth qualification is a first-release gate, independent of the later MCP server build.
