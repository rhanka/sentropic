...
**Consumer clarification, 2026-09-24 — Lot E revision round 2, following approve-with-fixes.** The mandatory h-cond inputs of 2026-09-23 (owner-confirmed flip, `01M38QMNYJTA3EYZTSV75KB7XM`) refine the acquisition and release plan above. The [Consumer contract for the h2a flip](SPEC_EVOL_CLUSTER_MESH_LAZY_SURFACE.md#consumer-contract-for-the-h2a-flip), E2–E10, is the detailed proposal; this clarification preserves D1, D3, D11 and D13–D16 and does not claim independent acceptance of the new fixes.

- **Same shape: yes.** Static `/llm-mesh`, `/llm-mesh/facade`, `/llm-mesh/enrollment`, `/llm-mesh/node` and `/gateway` re-export the accepted providers' original values/types. h2a substitutes import specifiers and retains synchronous calls after upstream version adaptations. Separate `/loaders/*` are additional asynchronous composition APIs, not a required consumer rewrite. The pinned `75c1dc61` table covers nine mesh files and five gateway matches; the fifth gateway match is a test comment, leaving four actual gateway import files.
- **Cluster-mesh owns laziness: yes, at the package/subpath boundary.** Optional peers and isolated root JS/declarations keep bare-root consumers independent. A selected static leaf evaluates its provider during ESM loading; absence fails native linking, while selected leaf types require provider declarations. Only loaders provide typed acquisition refusal and registry policy. Tree-shaking is not an installation guarantee, and h2a's existing eager runtime barrel still selects its LLM peers. This qualifies the earlier dynamic-import/refusal wording without promising per-call laziness for unchanged static imports.
- **Lot C auth isolation is preserved (round 2 / N2).** `/gateway` maps only to gateway root; its JS/types pull neither auth entry. Service mode selects `/gateway/auth` → `@sentropic/llm-gateway/auth` (`ServiceAuthVerifyToken` and types; peers mcp-auth and jose), with `/loaders/gateway/auth:loadGatewayAuth`. Session mode selects `/gateway/auth-hono` → `@sentropic/llm-gateway/auth-hono` (peer auth-hono), with `/loaders/gateway/auth-hono:loadGatewayAuthHono`. Service auth uses mcp-auth 0.2.1 (`>=0.2.1 <0.3.0`), oauth-verify 0.1.0, jose `^5.10.0` and Hono. Broken published mcp-auth 0.2.0 is excluded; publication of the 0.2.1 fix (`4a8251e1c`, under double review) is a B3 prerequisite, independent of the later MCP server build.
- **Auth preflight precedes bind (round 2).** Lot C D2 imports auth peers at verification time, so auth-leaf import alone does not prove startup readiness. `/compose/gateway` preloads the selected mode through its loader; static users must await the documented loader/equivalent qualified gateway hook before bind. Probes, version checks and preflight resolve those peers from gateway's physical package location. B2/B3 prove absent peer → startup failure, with no listener or first-request 401/500/503. h2a catches native-link rejection at its existing dynamic runtime import and catches preflight in that startup path; typed refusals are recognized by stable `code`, not `instanceof`.
- **Coherent upgrade, not an implicit peer bundle.** The first lazy minor targets 0.12.0 after 0.11.0 and Lot C gateway 0.18.0; accepted mesh/gateway ranges are `>=0.21.2 <0.22.0` and `>=0.18.0 <0.19.0`. Each release publishes an exact tested tuple, explicitly installed and lockfile-pinned by consumers. Every manifest declaring cluster-mesh and importing leaves also declares selected peers; the installed tree must have exactly one physical cluster-mesh and one mesh instance shared with gateway. h-cond moves h2a's 0.9 pin to 0.12.0 in the same artifact and qualifies the global consumer plus separately installed runtime under supported package managers, including peer-variant rejection. E9 retains ordered 0.10 contract checks/0.10.1 fixes and 0.11 feedback: `commandRef`, strict keys and verified custody precede import substitution. The tsup runtime build externalizes cluster root/all subpaths; native packed tests compare mesh identity from cluster and gateway. Loaders check runtime skew; static consumers use packed/install coherence gates.
- **Declaration safety follows the real consumer (round 2).** Provider leaves use only `export *`, never named peer re-exports that can silently become `any` under h2a's `skipLibCheck: true`. B3 crosses true/false with NodeNext/Bundler and installed/absent peers; the skipped-library case expects a consumer missing-export diagnostic. Enrollment remains a full static namespace even when the consumer uses only its `AccountPublic` type.
- **MCP covered, built later: yes.** B1–B3 qualify/release the LLM/gateway surface; B4 ships that h2a flip before B5 starts MCP adapters. B5–B7 qualify a subsequent MCP/catalog minor, then B8 migrates MCP. Retain the 0.97.3 startup and 0.97.4 identity-decoupling regression gates and one logical MCP authority. Existing gateway service auth does not advance the MCP build. The universal catalog remains the target; later/private capabilities are explicitly gated until qualified.
- **Review and delivery remain gated.** Opus 5.5 returned approve-with-fixes (architecture holds, four majors, no blocker); round 2 addresses those findings. The conductor obtains independent acceptance of the fixes before the approved build handoff to h-cond and the product conductor. The requested topology rule/test notice is informational; h-cond builds nothing before that approved handoff. Lot E changes only planning documents; no implementation, final review pass or release is asserted here.
- **Implementation status, 2026-09-24.** cluster-mesh 0.12.0 implements the LLM/gateway lazy surface (B1–B3 package side) on `spec/cluster-mesh-lazy-surface`, adding the conductor-decided per-process topology guard described in the lazy-surface spec. Publication, independent review and the h2a flip remain gated; MCP is not yet built.
