...
**Consumer clarification, 2026-09-24 — Lot E revision round 1, pending design review.** The mandatory h-cond inputs of 2026-09-23 (owner-confirmed flip, `01M38QMNYJTA3EYZTSV75KB7XM`) refine the acquisition and release plan above. The [Consumer contract for the h2a flip](SPEC_EVOL_CLUSTER_MESH_LAZY_SURFACE.md#consumer-contract-for-the-h2a-flip), E2–E10, is the detailed proposal; this clarification preserves D1, D3, D11 and D13–D16 and does not claim a new review approval.
...
- **Coherent upgrade, not an implicit peer bundle.** The first lazy minor targets 0.12.0 after 0.11.0 and Lot C gateway 0.18.0; accepted mesh/gateway ranges are `>=0.21.2 <0.22.0` and `>=0.18.0 <0.19.0`. Each release publishes an exact tested tuple, explicitly installed and lockfile-pinned by consumers, with one physical mesh instance. E9 orders h2a's existing 0.9 pin through 0.10 contract checks/0.10.1 fixes, 0.11 feedback and the lazy minor: `commandRef` wire mapping, strict keys and verified custody precede import substitution. Loaders check runtime skew; static consumers use packed/install coherence gates.
...
- **Review and delivery remain gated.** The conductor obtains a passing independent Opus 5.5 design review of the revised contract before delivering it to h-cond and the product conductor. h-cond builds nothing before that handoff. Lot E changes only planning documents; no implementation, review pass, consumer delivery or release is asserted here.
