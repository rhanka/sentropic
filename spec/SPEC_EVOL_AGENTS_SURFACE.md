...
**D22 — The list items are real design-system components, behind an OPTIONAL peer dependency scoped to the agents subpath (owner ratification, 2026-07-30).** Established first: `packages/chat-ui/package.json` declared **no** DS dependency at all (peers were only `@lucide/svelte`, `svelte`, `svelte-streamdown`), which is why every chat-ui component is hand-written Tailwind while only its packaged theme consumes `--st-*`. The DS provides exactly the primitives this surface needs — `SelectableList`/`SelectableRow` (the list owns selection + roving tabindex, rows are `option` in a `listbox`), `StatusDot` (semantic tones on `--st-semantic-feedback-*`, `pulse` for a live state, disabled under `prefers-reduced-motion`, `label` turning it into a full status indicator), `Tag`/`Badge`, `Avatar`/`Initials`, `OverflowMenu`, `Menu`, `NavRail`, `Tabs`. `EventFeedPanel` was considered and rejected: it sorts strictly by date descending and cannot carry R9's bucketed order.
So: `@sentropic/design-system-svelte@^0.34.0` becomes an **optional** peer (`peerDependenciesMeta`), and only the agents-surface components import it, on their own export subpath. A bundler resolves only what it imports, so sentropic and radar get identical DS items while a plugin host that never mounts the list pays nothing. Precedent for the peer pattern: `auth-ui@0.7.1` declares the DS as a required peer.
Consequence recorded as scope exception **CHAT-AGENTS-EX1**: `test-chat-ui-dom` installs a fixed package list, so the DS had to be added there or the component could not be DOM-tested at all — `auth-ui` ships its DS components with no DOM coverage, which is below this package's bar (13 suites, 189 tests). Verified: the DS resolves in the sandbox and all 189 pre-existing DOM tests stay green.
