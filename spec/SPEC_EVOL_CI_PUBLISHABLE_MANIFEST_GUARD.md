...
| `enforce-package-bump` is PR-only and considers `packages/<p>/src/**` | Keep it unchanged; the new BLOCK guard compares packed dependency maps to the existing published version and fails with "bump required" on differences (section 8). |
...
| D7 | Existing-version BLOCK candidates with changed packed dependency maps require a bump | A dependency-only repair must not pass CI and then silently skip publication. |
| D8 | Inject guarded same-PR BLOCK sibling archives for candidate qualification | Lockstep bumps must not fail solely because a sibling is not yet on the registry; published qualification remains registry-only. |
...
| `check-publishable-manifests` | Always enumerate `U`, validate context, check all source manifests, run full `make pack-<slug>` sequentially for BLOCK packages, compare their packed dependency maps with any existing exact published version, and perform lightweight archive audits for WARN packages. Aggregate every diagnostic before the final exit. |
...
| `qualify-published-install PKG=<name>@<exact-version>` or `qualify-published-install TARBALL=<path> [SIBLING_ARCHIVES_FILE=<path>]` | Install and import in a fresh consumer container as specified in section 10. Exactly one primary input is required; the optional sibling receipt list is restricted to same-PR candidate qualification. |
...
Dependency-only bump gate: for each BLOCK candidate, query its exact packed name/version inside Docker with section 6's error semantics. If that version exists, fetch its published tarball using registry `dist` metadata, verify integrity, and compare its `dependencies`, `peerDependencies`, and `optionalDependencies` independently with the candidate's packed maps. Sort keys and treat an absent map as empty; compare names and string values exactly, without semver-equivalence normalization. Any addition, removal, or changed value fails with `bump required`, naming the version, changed section/entry, and both artifact hashes. An identical map set adds no bump failure; other manifest violations still fail. Fetch/integrity/parse failure is ERROR, not a bypass. Run this in the shared BLOCK candidate guard and required inventory job before accepting the candidate; it is not postponed to the publisher, which skips existing versions. WARN packages remain warning debt. Thus a manifest-only `file:` → `^x` repair without a bump cannot pass the PR gate while leaving the broken registry version unchanged; `enforce-package-bump` stays unchanged.
...
6. Add `qualify-published-install` steps to `validate-mcp-auth` and `validate-cluster-mesh` after their full packs, under `if: steps.pack.outputs.manifest_mode == 'block'`, using `steps.pack.outputs.tarball`. Add `PACK_DESTINATION` and `PACK_OUTPUT_FILE` to retain and identify those exact job-local archives, never a glob. For PR candidates, use the shared Make orchestrator to pack their relevant BLOCK siblings from the same PR head and pass the guarded receipt list as `SIBLING_ARCHIVES_FILE` (section 10); no dependency on the global job is needed. An unrelated validation must not turn a WARN manifest into a blocking clean-install failure. Post-publish qualification steps in `publish-mcp-auth`, `publish-cluster-mesh`, and selected bootstrap paths install the exact packed version from the receipt, using registry artifacts only. Retry a confirmed missing just-published package/sibling version up to 12 attempts, 5 seconds apart; a remaining `pending-sibling-publish` blocks after publication. Other lookup/install/import failures remain failures. Upload each qualification report under `if: always()`.
...
`qualify-published-install` uses `$(LLM_MESH_NODE_IMAGE)`, the Node 24 Debian slim image already used by these packages. Start a new `docker run --rm` via Make with no Compose dependencies, host Node, Python, exposed ports, npm credentials, monorepo mount, or shared node_modules/cache. Mount only the standalone probe script read-only at `/probe/qualify.mjs`, an optional primary archive read-only at `/input/package.tgz`, individually allowlisted sibling archives/receipts read-only for PR candidates, and an explicit report directory at `/reports`. The image's bundled Node/npm is logged with the resolved image identity. Never mount the repository at a second path or copy its lockfile/overrides into the consumer.
...
Same-PR sibling contract: before installing a BLOCK candidate, pack the relevant BLOCK siblings in its dependency closure from the same head SHA using their explicit full pack targets, sequentially and once per job. `SIBLING_ARCHIVES_FILE` lists their names, packed versions, absolute archive paths, SHA-256 hashes, head SHA, and strict guard receipts. Validate every receipt/archive and original dependency range before injecting these as local archive inputs into the disposable consumer; never use workspace links or rewrite packed dependency maps. Where transitive resolution requires an override, generate it only in the disposable consumer, only for the supplied sibling and edges whose original ranges accept that version. Record the override and verify the installed tree uses the supplied archives at those edges; conflicts or incompatible ranges fail. Keep optional peers out of the initial core smoke, including sibling optional peers; inject them only in the explicit peer phase. A successful run is `candidate-with-siblings`, not evidence that those versions can yet be installed from the registry. `PKG` replays and every post-publication check reject sibling injection and consumer overrides.

For remaining confirmed missing sibling versions awaiting publication, emit `pending-sibling-publish` with package/range, intended version, PR/publication evidence, and unresolved imports. This is non-blocking only in PR qualification and is never reported as an install/import PASS; after publication it is blocking once the bounded propagation retries are exhausted. Do not turn arbitrary registry errors, missing external dependencies, incompatible ranges, invalid sibling manifests, missing mandatory pack lanes, or failed sibling builds/packs into pending status. Registry lookup failure remains ERROR. The required pinned published replays cannot close BUILD with a pending result.
...
Optional peer contract: optional peers must not be injected before the core smoke, which could conceal an accidental eager import. First install the core package inputs, including required candidate siblings if supplied, without optional peers and import the root entry. If `PEERS` is supplied, accept only explicit exact versions declared in this package's peerDependencies and satisfying those ranges; resolve from the registry, or a matching guarded sibling receipt in PR candidate mode only. Add them inside the same clean consumer, then import ALL runtime entrypoints. Log before/after dependency trees and every added peer. Without `PEERS`, still attempt every entrypoint; missing optional peers produce an actionable failure rather than a success with skipped coverage. For mcp-auth's optional Hono adapter, supply `PEERS=hono@4.10.7`; jose is a required peer and npm resolves it normally. cluster-mesh needs no peer override.

Output: one JSON record plus human summary with requested/resolved input, archive hash/integrity, date/commit/job, image/Node/npm versions, clean consumer path, npm command/exit, dependency tree, core import result, optional peers added, sibling receipts/overrides, each entrypoint/condition and result, and final pass/fail/pending status. Preserve logs/reports outside the disposable container; remove it on success/failure. Except for the narrowly defined PR pending status above, installation failure, incorrect identity, escaped resolution, missing entrypoint, import exception, or timeout exits nonzero. Success means this Node consumer installed/imported the stated artifact and recorded sibling inputs; it does not certify application semantics, all platforms, or browser integrations.
...
| `scripts/ci/publishable-pack.test.mjs` (bump cases) | Manifest-only `file:` → semver repair at an existing version fails `bump required`; additions/removals/changes in each runtime/peer/optional map fail; reordered keys/absent vs empty maps pass comparison; a bumped absent version passes; published artifact lookup/integrity failures ERROR; dev-only changes do not demand a bump here. |
...
| `scripts/ci/qualify-published-install.test.mjs` | Good local fixture imports, absent dist/misdeclared export/imported missing dependency fails even if install succeeds, workspace-only sibling cannot resolve, guard rejects file dependency, name/version mismatch, optional peer core independence plus adapter coverage, multiple exports/CJS, unsupported patterns, invalid inputs, failed registry resolution, report retention and cleanup. Same-PR lockstep bump installs direct/transitive sibling archives; hashes/head/range mismatches and failed packs block; optional sibling peers stay out of core smoke; pending is non-blocking only in PR and blocks after publication; registry replays reject injection. |
...
Acceptance: no pack or publication path omits the checker; source and packed values follow D1; global-only PRs do not fail because of unrelated manifest debt; actual selected offenders block; dependency-only packed-map changes at an existing version fail with `bump required`; all changed public packages are discovered; lockstep PR candidates use guarded sibling archives or report a precise pending status; post-publication checks and both published replays require registry installation success; lint reaches cluster-mesh CI. Local tests cannot claim GitHub branch protection or OIDC publication was exercised: conductor CI review supplies workflow evidence without a test publication.
