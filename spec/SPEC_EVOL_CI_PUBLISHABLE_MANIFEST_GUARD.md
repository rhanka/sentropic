...
| `scripts/ci/publishable-classification.test.mjs` | Source/test/README/manifest-only edits; new/public-transition/deleted/renamed packages; main multi-commit push; dispatch explicit/all/none; global-only vs root-lock selection; no version-exists escape; missing/corrupt context; focus's unused steady-state filter; public skills inventory. |
...
| `scripts/ci/publishable-ci-wiring.test.mjs` | Parse real YAML with pinned ephemeral `yaml@2.8.1`; assert filter-output and publisher mapping coverage, strict pack path for every selected package, token/OIDC guards, gateway `always()` success check, new required job scheduling, lint step, context passed to validation packs, and no credential exposure in validation. A stub publisher must receive the exact inspected hash, never the directory. |
...
Exercise the auth incident with a local fixture reproducing `file:../oauth-verify`, plus a plain-semver replacement. Do not depend on an immutable broken registry release for deterministic tests. Use the two exact published replays for real registry evidence. Known cowork source violations must appear as warnings in a global-only context and errors when cowork is touched or bootstrap-selected. A root-lock edit intentionally selects cowork for BLOCK; do not change publish filters just to evade this consequence.
...
| BRCI-EX2 — `.github/workflows/ci.yml` | Classification outputs, always-scheduled guard, validation context, lint, publisher/bootstrap needs, install smoke steps | New required check and registry qualification; global-only WARN behavior; no new publish permission or trigger | Revert job/step/needs/filter hunks together with EX1; conductor removes the required status before removing its job. Keep existing version-bump gate and publication ordering. |
...
5. Conductor CI review: run the listed gates and both published replays, attach reports, confirm annotations and publisher `needs` behavior, then require the new status check. The conductor handles push/PR/merge; only normal CI can publish. No manual npm publication is part of rollout or testing.
...
Risks and limits: new inventory packaging adds registry/tooling latency; BLOCK candidates can build twice across validation and inventory jobs (accepted for the initial conservative rollout). Preserve unique artifact paths and serialize package transformations within a worktree. Snapshot audits intentionally do not execute unrelated pack hooks; final full packs and strict publication cover those mutations. Any future unsupported lifecycle or new public package lane must fail explicitly until integrated. Root manifest/lockfile edits already fan out publication and can expose cowork's debt. Successful imports prove Node loadability only; no native desktop/browser/CLI behavior is certified. No live install result or peer consensus is claimed in this SPEC.
