...
| `@sentropic/auth-ui` (BR-39a) | scaffolded, branch-local | reusable Sentropic-style authentication screens + browser passkey helpers; exports `AuthUiTransport` shape consumed by `auth-hono` route contracts | server logic (→ `auth-hono`); persistence; product copy/locale ownership |
