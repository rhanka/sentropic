...
| `@sentropic/auth-hono` (BR-39b + BR-39c) | published 0.3.0 | reusable Hono route factories for email-code, magic-link, passkey registration/authentication, sessions, credentials; pure auth services (WebAuthn ceremony, sessions, email/magic-link, account policy) backed by `AuthHonoPorts`; route-handler hooks for app-owned policy (`prepareRegistrationOptions`/`resolveRegistrationUser`/`resolveAuthenticationOptions` short-circuit `{ status, code, message }`; `finalizeRegistration`/`finalizeAuthentication` post-verify response control); structured response contract (`{ delivery, expiresAt, success }` / `{ error: { code, message } }`); `createRequireAuth`/`createOptionalAuth` middleware factories that do not assume host workspace model; **+ OAuth2/OIDC IdP surface (BR-39c)**: `GET /oauth/authorize`, `POST /oauth/token`, `GET|POST /oauth/userinfo`, `POST /oauth/revoke`, `POST /oauth/introspect`, `POST /oauth/consent/decision` (host-private); root-mounted `GET /.well-known/openid-configuration` + `GET /.well-known/jwks.json`; PKCE-only authorization-code grant (S256 required); Ed25519 / EdDSA JWT signing with JWKS rotation via `JwksService`; DPoP opt-in per client (RFC 9449, `dpop_bound_access_tokens` flag); `OauthStateStorePort` + `JwksPort` for host-supplied storage abstraction; `acr` + `auth_time` claims emitted from passkey sessions for BR-39j step-up consumers; `tenant_id` forward-compat column (BR-39e); mock RP E2E coverage via `packages/auth-hono/tests/example-oauth-rp.test.ts` | persistence (host adapters supply Drizzle/file/memory ports); cookie attribute policy (host decides `Secure`/`Domain` per env); session-creation policy (host owns via `finalize*` — e.g. Sentropic injects workspace bootstrap + first-admin + account-status into the session it issues); product email copy; rate limiting middleware (host-owned); consent grant persistence (→ BR-39g admin UI); multi-tenant enforcement (→ BR-39e) |
| `@sentropic/auth-ui` (BR-39a + BR-39c) | published 0.3.0 | reusable Svelte 5 authentication screens (`AuthLogin`, `AuthRegister`, `AuthMagicLinkVerify`, `AuthDevices`, `AuthDevicePair`) + browser passkey helpers + `AuthUiTransport` (13 methods: email-code/magic-link/passkey-register/passkey-login/session-refresh/logout/credentials list+rename+revoke/device-pair-approve); `createDefaultFetchTransport({baseUrl, fetch?, headers?, onUnauthorized?})` for `@sentropic/auth-hono`-compatible mounts; `createDefaultAuthUiLabels` (EN) + `createFrenchAuthUiLabels` (FR) presets, all overridable via `Partial<AuthUiLabels>`; visual customisation via CSS custom properties + named slots (`no-account`/`register-new-device`/`back-to-login`/`back-to-devices`/`pair-cta`/`add-device`/`login-link`/`cancel`); `AuthRegister.skipEmailVerification` for hosts owning pre-auth (e.g. SSO/admin invite); Sentropic UI rewired to thin wrappers (~25 LOC/route) via `ui/src/lib/services/auth-transport.ts` adapter; **+ OAuth consent + RP client helper (BR-39c)**: `<OAuthConsent />` Svelte 5 component with `state`, `transport` (dedicated `OAuthConsentTransport`), `labels?`, `onRedirect?` props and `branding`/`scope-description`/`footer` slots; `createOAuthClient({ issuer, clientId, redirectUri, scopes, dpop? })` for discovery caching, PKCE keypair generation (`code_verifier`/`code_challenge`), `exchangeCode`, `revoke`, `userInfo`, and optional DPoP Ed25519 keypair management via SubtleCrypto; `createDefaultOAuthConsentLabels` (EN) + `createFrenchOAuthConsentLabels` (FR); `peerDep @sentropic/auth-hono: ^0.3.0` | server logic (→ `auth-hono`); persistence; host session store + navigation (callbacks); product copy / locale store ownership (labels passed in); rate limiting (host); brand assets (slots + CSS vars); per-tenant branding (→ BR-39f); admin OAuth client management (→ BR-39g) |
