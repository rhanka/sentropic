...
| Gemini `gemini-3.5-thinking` | Generate Content `parts[].text`, `parts[].thought`, `parts[].functionCall`. | text without `thought` -> `content_delta`; `thought: true` -> `reasoning_delta`; `functionCall` -> `tool_call_start`. | `gemini-tool-handoff` request-body tests; `llm-runtime-stream` matrix content/tool/reasoning row. |
