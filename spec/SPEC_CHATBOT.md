...
> **Summary:** 23 use cases identified (CU-001 to CU-023) covering: object modifications, history, streaming reasoning, replay, context management, model switching, deepening, structured calls vs informal sessions, object creation via chat, consultation/search, cancellation/rollback, multi‑context, suggestions, export/sharing, session management, view integration, long context, validation/confirmation, queue integration, notifications/feedback, error handling, document context, and generated image media.
...
- [x] **CU-023: Generated image media in chat** (BR-38b)
  - [x] Tool `image_generate` creates one or more images from a prompt.
  - [x] Generated bytes are stored as ready chat-session `context_documents`, not embedded in assistant text.
  - [x] Chat renders document-backed generated image cards with preview, metadata, and download actions.
  - [x] Unsupported/planned provider paths surface deterministic tool-result errors.
  - [ ] Reuse generated images as later BR-38a image attachments once the image-input contract is active on `main`.
...
- The API runtime imports `@sentropic/llm-mesh` as a workspace package and routes `callLLM` / `callLLMStream` through the mesh-backed `api/src/services/llm-runtime` boundary.
- `@sentropic/llm-mesh` provides the public TypeScript contract for provider/model IDs, model capability profiles, normalized generation and streaming events, tool calls, structured output, retryable provider errors, and redacted auth descriptors.
...
## Image generation tool and generated media lifecycle (BR-38b)

`image_generate` is a server-side chat tool enabled for `ai-ideas` and `opportunity` workspaces. The model should use it when the user asks to create, generate, design, or render an image; the assistant must not paste base64 image bytes in chat content.

Tool schema:

- Required: `prompt`.
- Optional: `providerId` (`openai`, `gemini`, `mistral`, `anthropic`, `cohere`), `model`, `count` (1 to 4), `size`, `aspectRatio`, `quality`, and `background`.
- Provider/model defaults come from the image-capable runtime path: OpenAI defaults to `gpt-image-2`; Gemini defaults to `gemini-3.1-flash-image-preview`.
- Anthropic and Cohere fail before dispatch with `unsupported_provider`; Mistral fails before dispatch with `planned_provider_runtime` until an Agents/Conversations image adapter exists.

Execution flow:

1. Chat tool dispatch validates the prompt and generation controls.
2. The API calls `generateImage()` through the mesh-backed `api/src/services/llm-runtime` boundary.
3. OpenAI and Gemini responses must produce base64 image bytes for this storage path.
4. Each generated image is written to object storage and inserted into `context_documents` with `context_type='chat_session'`, `context_id=<sessionId>`, `status='ready'`, `source_type='local'`, and `data.indexingSkipped=true`.
5. `data.generatedMedia` records image metadata: prompt, tool call ID, user ID, generation ID, provider, model, dimensions, and provider metadata.
6. The tool result returns `{ status: 'completed', media: [...] }` with document IDs, filenames, MIME types, dimensions, provider/model metadata, prompt, and document-backed download URLs.
7. Chat rendering rebuilds preview/download URLs from encoded `documentId` values and ignores untrusted external URLs in stream payloads.

Error semantics:

- `unsupported_provider`: provider/model has no supported image generation path.
- `planned_provider_runtime`: provider support requires a runtime adapter not implemented by this branch.
- `missing_credentials`: provider credentials are unavailable.
- `invalid_generation_options`: requested controls are unsupported by the selected provider/model.
...
- `provider_failure`: provider returned an unexpected or transient failure, or no usable image bytes.
...
6. **Generated images** → chat tool `image_generate` stores generated media as ready chat-session documents with `data.indexingSkipped=true` and `data.indexingSkipReason='generated_image'`; downloads use the same `/api/documents/:id/content` access-control path.
...
- [x] Generated image documents from `image_generate` are stored as ready chat-session documents and excluded from summary/indexing.
