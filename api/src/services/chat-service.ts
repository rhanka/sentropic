...
const DEFAULT_FREEFORM_PPTX_CODE = `
const entity = context.entity || {};
const title = safeText(entity.name, "Generated presentation");
const status = safeText(entity.status, "n/a");
const folderCount = Array.isArray(context.folders) ? context.folders.length : 0;
const initiativeCount = Array.isArray(context.initiatives) ? context.initiatives.length : 0;
const deck = pptx({ title });
const slide = titleSlide(deck, title, "Context summary");
bullets(slide, [
  "Status: " + status,
  "Folders: " + folderCount,
  "Initiatives: " + initiativeCount,
], { x: 1.2, y: 4.15, w: 10.8, h: 1.25, fontSize: 16 });
...
For a minimal one-slide PPTX summary, it is acceptable to call \`document_generate\` with \`format: "pptx"\`, \`title\`, and the current target but without \`code\`; the server will generate the minimal slide from the current context.
    ...
    switch (toolCall.name) {
      ...
      case 'document_generate': {
...

        {
          ...
          if (!templateId && !code) {
            if (format === 'pptx') {
              code = DEFAULT_FREEFORM_PPTX_CODE;
            } else {
              throw new Error('document_generate: either templateId or code is required');
            }
          }
