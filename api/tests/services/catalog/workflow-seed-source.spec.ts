...
describe('0-regression — workflow entries absent from resolveFoundationChatTools', () => {
  ...
  it('the tool count oracle is correct after wiring the workflow source (Lot 7: 2 meta + foundation)', () => {
    // Lot 7 update: search_catalog is at index 1. Workflow source adds no tools.
  ...
  it('the exact tool name sequence includes search_catalog at index 1 (Lot 7 update)', () => {
    // Lot 7 update: workflow entries cause ZERO change to the foundation tool sequence.
    // search_catalog is now at index 1. Workflow entries do NOT appear.
    const tools = resolveFoundationChatTools({
      userId: 'u-lot4-test',
      workspaceId: 'ws-lot4-test',
      workspaceType: 'ai-ideas',
